...
import folio_docs.build as build_module
import folio_docs.branding as branding_module
import folio_docs.config as config_module
from folio_docs import __version__
from folio_docs import cli as cli_module
from folio_docs.cli import _generate_docs_yaml, _sync_version_matrix, app
...
def test_cli_version_names_the_shared_command() -> None:
    result = runner.invoke(app, ["--version"])

    assert result.exit_code == 0
    assert result.output.strip() == f"folio {__version__}"


def test_cli_loads_the_installed_agents_product() -> None:
    command = get_command(app)

    assert "board" in command.commands
...
def test_cli_init_quotes_pyproject_metadata_into_docs_yaml(
    tmp_path: Path, field: str, hostile: str
) -> None:
    """A cloned repo must not be able to inject config keys nobody listed.

    ``folio init`` reads ``project.name``/``project.version`` from the target
    repo's pyproject.toml. Both are attacker-controlled in a clone-and-init
    flow, and a ``plugins:`` key smuggled in through either one would execute
    arbitrary code on the next ``folio`` command.
    """
    fields = {"name": "safe-name", "version": "0.1.0"}
    fields[field] = hostile
    (tmp_path / "pyproject.toml").write_text(
        "[project]\n"
        f"name = {json.dumps(fields['name'])}\n"
        f"version = {json.dumps(fields['version'])}\n",
        encoding="utf-8",
    )
    ...
    assert parsed["project"][field] == hostile


def test_cli_init_quotes_git_remote_into_docs_yaml(
    tmp_path: Path, monkeypatch
) -> None:
    """``project.repo`` comes from .git/config, which a clone also controls."""
    hostile = 'https://example.com"\nplugins: ["docs/evil.py"]\ntrailing:\n  key: "y'
    monkeypatch.setattr(cli_module, "_detect_git_remote", lambda target: hostile)
    ...
    parsed = yaml.safe_load((tmp_path / "docs.yaml").read_text(encoding="utf-8"))
    assert "plugins" not in parsed
    assert parsed["project"]["repo"] == hostile
...
def test_cli_build_reports_plugin_hook_failures_as_build_failures(
    ...
    from folio_docs.plugin import PluginHookError
...
def test_cli_build_reports_python_syntax_errors_as_build_failures(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(build_module, "run_build", raise_syntax_error)

    result = runner.invoke(app, ["build", "--project-dir", str(tmp_path)])

    assert result.exit_code == 1
    assert "Build failed: invalid source" in result.output


def test_cli_serve_reports_python_syntax_errors_as_build_failures(
    tmp_path: Path,
    monkeypatch,
) -> None:
    (tmp_path / "docs.yaml").write_text(
...
def test_sync_version_matrix_includes_current_plugins_and_plugin_config(
    ...
    plugin_config = {
        "plugins": ["folio_docs.docs.integrations.roadmap"],
        "roadmap": {
    ...
    assert raw["plugins"] == ["folio_docs.docs.integrations.roadmap"]
