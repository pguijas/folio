...
def test_generate_llms_txt_uses_exported_route_paths():
    ...
    modules = [
        ModuleIR(
            name="folio_docs.agent_output.llm_output",
    ...
    assert (
        "[folio_docs.agent_output.llm_output](/docs/api-reference/folio_docs/agent_output/llm_output/)"
        in result
    )
    assert "/components/index" not in result
    assert "/api-reference/folio_docs.agent_output.llm_output" not in result
...
def test_generate_llms_txt_skips_disabled_api_modules(monkeypatch):
    ...
    monkeypatch.setattr(
        features, "MVP_DISABLED_API_MODULES", {"folio_docs.docs.integrations.roadmap": "roadmap"}
    )
    config = _make_config()

    result = generate_llms_txt(config, [_make_disabled_module()], [])

    assert "folio_docs.docs.integrations.roadmap" not in result
    assert "/docs/api-reference/folio_docs/docs/integrations/roadmap/" not in result
...
def test_generate_llms_full_txt_escapes_pipes_in_type_and_default_cells():
    """A PEP 604 union must not split its own table row.

    ``_table_cell`` escaped the description only, so ``str | None`` injected a
    raw pipe into the Type cell: the row grew a fifth cell, the type was cut in
    half and the description fell off the end of the table.
    """
    func = FunctionIR(
        name="lookup",
        args=[
        ...
        raises=[],
        decorators=[],
        docstring=DocstringIR(short_description="Look something up."),
        is_async=False,
        source_file="lookup.py",
        line_number=1,
    )
    module = ModuleIR(
        name="lookups",
        docstring=DocstringIR(short_description="Lookups."),
        classes=[],
        functions=[func],
        constants=[],
        source_file="lookup.py",
    ...
    result = generate_llms_full_txt([module], [])

    assert "| `key` | `str \\| None` | `None` | The key. |" in result
    assert (
        "| `mode` | `Literal['a'] \\| Literal['b']` | `'a' \\| 'b'` | The mode. |"
        in result
    )
    header_cells = "| Parameter | Type | Default | Description |".count("|")
    for line in result.splitlines():
        if line.startswith("| `key`") or line.startswith("| `mode`"):
            assert line.replace("\\|", "").count("|") == header_cells
