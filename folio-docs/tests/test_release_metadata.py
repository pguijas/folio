...
def test_wheel_includes_bundled_template_workspace() -> None:
    ...
    assert force_include["template/package.json"] == "folio_docs/template/package.json"
    assert force_include["template/pnpm-lock.yaml"] == "folio_docs/template/pnpm-lock.yaml"
    assert force_include["template/pnpm-workspace.yaml"] == (
        "folio_docs/template/pnpm-workspace.yaml"
    )
    assert force_include["template/app"] == "folio_docs/template/app"
    assert force_include["template/components"] == "folio_docs/template/components"
    assert force_include["template/theme"] == "folio_docs/template/theme"
    ...
    assert all(
        target.startswith("folio_docs/template/") for target in force_include.values()
    )
...
def test_standalone_installer_is_documented_and_valid_shell() -> None:
    ...
    for content in (readme, installation):
        ...
        assert f"curl -LsSf -o install-folio-docs.sh {install_url}" in content
...
def test_docs_use_publishable_distribution_name() -> None:
    pyproject = tomllib.loads((ROOT / "pyproject.toml").read_text())
    installer = (ROOT / "install.sh").read_text()
    docs_yaml = (REPO_ROOT / "docs.yaml").read_text()
    checked_paths = [
        ROOT / "README.md",
    ...
    assert "if have folio; then" in installer
    assert "folio is not on PATH yet" in installer
    assert (
...
def test_python_package_metadata_is_configured() -> None:
    ...
    assert (ROOT / "folio_docs" / "py.typed").exists()
...
def test_ci_runs_python_and_template_checks() -> None:
    ...
    assert "branches: [develop, main]" in workflow_text
    ...
    assert "uv run --locked ruff check folio-docs folio-agents" in step_text
    assert "uv run --locked pytest" in step_text
    assert "uv build --project folio-docs" in step_text
    assert "uv build --project folio-agents" in step_text
...
def test_branch_preview_workflow_deploys_folio_docs_to_github_pages() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "branch-previews.yml"
    workflow_text = workflow_path.read_text(encoding="utf-8")
    workflow = yaml.safe_load(workflow_text)
...
def test_branch_previews_are_garbage_collected_on_deploy() -> None:
    ...
    workflow_dir = REPO_ROOT / ".github" / "workflows"
    assert not (workflow_dir / "branch-preview-cleanup.yml").exists()
...
def test_release_workflow_publishes_to_pypi_with_trusted_publishing() -> None:
    workflow_path = REPO_ROOT / ".github" / "workflows" / "release-docs.yml"
    workflow_text = workflow_path.read_text(encoding="utf-8")
    workflow = yaml.safe_load(workflow_text)

    assert workflow["name"] == "Release Folio Docs"
    assert "docs-v*" in workflow_text
    assert workflow["permissions"] == {"contents": "read"}
    ...
    assert "uv sync --all-groups --locked" in build_text
    assert (
        "uv run --locked python folio-docs/scripts/check_release_version.py"
        in build_text
    )
    assert "uv run --locked ruff check folio-docs" in build_text
    assert "uv run --locked pytest folio-docs/tests" in build_text
    assert "node-version': 24" in build_text
    assert "uv run --locked folio build --clean" in build_text
    assert "rm -rf dist" in build_text
    assert "uv build --project folio-docs" in build_text
...
def test_published_github_links_point_at_the_canonical_repo() -> None:
    """Every published GitHub link must use the repo `project.repo` names.

    The landing config and the guide are easy to point at a `pguijas/*`
    repository other than the one `project.repo`, `pyproject`, and every
    generated `[source]` link use. Folio's link checker only validates
    internal links, so nothing else catches a dead external CTA.
    """
    canonical = "https://github.com/pguijas/folio"
    other_repo = re.compile(r"https://github\.com/pguijas/(?!folio(?:[/\s)\"']|$))\S+")

    # docs.yaml sits at the repository root, not inside the package — the two
    # other tests in this file already read it from REPO_ROOT. This one kept
    # the pre-split path, where the package root and the repo root were the
    # same directory.
    published = [
        REPO_ROOT / "docs.yaml",
        ROOT / "README.md",
    ...
    for path in published:
        for number, line in enumerate(
            ...
            for match in other_repo.finditer(line):
                offenders.append(
                    f"{path.relative_to(REPO_ROOT)}:{number}: {match.group(0)}"
                )

    assert not offenders, (
        f"published links must use {canonical}: " + "; ".join(offenders)
    )
