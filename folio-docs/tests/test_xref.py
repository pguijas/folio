...
from folio_docs.docs.xref import build_symbol_index, resolve_type_link
...
class TestBuildSymbolIndex:
    def test_indexes_modules(self):
        ...
        assert "folio_docs.config" in index
        assert index["folio_docs.config"] == "/docs/api-reference/folio_docs/config"

    def test_uses_configured_docs_route_base(self):
        index = build_symbol_index(_sample_modules(), docs_route_base="/reference/docs")

        assert index["folio_docs.config"] == "/reference/docs/api-reference/folio_docs/config"
        assert (
    ...
    def test_indexes_classes(self):
        index = build_symbol_index(_sample_modules())
        assert "folio_docs.config.Config" in index
        assert index["folio_docs.config.Config"] == "/docs/api-reference/folio_docs/config#config"

    def test_indexes_functions(self):
        index = build_symbol_index(_sample_modules())
        assert "folio_docs.config.load_config" in index
        assert (
            index["folio_docs.config.load_config"]
            == "/docs/api-reference/folio_docs/config#load_config"
        )

    def test_indexes_inner_classes(self):
        index = build_symbol_index(_sample_modules())
        assert "folio_docs.ir.ClassIR.Meta" in index
        assert index["folio_docs.ir.ClassIR.Meta"] == "/docs/api-reference/folio_docs/ir#meta"
...
class TestResolveTypeLink:
    ...
    def test_fully_qualified_name(self):
        url = resolve_type_link("folio_docs.config.Config", self.index, "folio_docs.build")
        assert url == "/docs/api-reference/folio_docs/config#config"

    def test_simple_name_in_current_module(self):
        url = resolve_type_link("Config", self.index, "folio_docs.config")
        assert url == "/docs/api-reference/folio_docs/config#config"

    def test_simple_name_search_all_modules(self):
        # Config is unique across all modules, so it should be found
        url = resolve_type_link("Config", self.index, "folio_docs.build")
        assert url == "/docs/api-reference/folio_docs/config#config"

    def test_generic_type_list(self):
        url = resolve_type_link("list[Config]", self.index, "folio_docs.build")
        assert url == "/docs/api-reference/folio_docs/config#config"

    def test_generic_type_optional(self):
        url = resolve_type_link("Optional[Config]", self.index, "folio_docs.build")
        assert url == "/docs/api-reference/folio_docs/config#config"

    def test_union_type_with_none(self):
        url = resolve_type_link("Config | None", self.index, "folio_docs.build")
        assert url == "/docs/api-reference/folio_docs/config#config"

    def test_dict_with_two_known_types_returns_none(self):
        # dict[ModuleIR, Config] has two known types -- ambiguous
        url = resolve_type_link("dict[ModuleIR, Config]", self.index, "folio_docs.build")
        assert url is None

    def test_dict_with_builtin_key(self):
        # dict[str, Config] -- str is builtin, Config is the only candidate
        url = resolve_type_link("dict[str, Config]", self.index, "folio_docs.build")
        assert url == "/docs/api-reference/folio_docs/config#config"

    def test_builtin_type_returns_none(self):
        url = resolve_type_link("str", self.index, "folio_docs.config")
        assert url is None

    def test_none_type_returns_none(self):
        url = resolve_type_link("None", self.index, "folio_docs.config")
        assert url is None

    def test_unknown_type_returns_none(self):
        url = resolve_type_link("SomethingUnknown", self.index, "folio_docs.config")
        assert url is None

    def test_empty_string_returns_none(self):
        url = resolve_type_link("", self.index, "folio_docs.config")
        assert url is None

    def test_list_of_builtin_returns_none(self):
        url = resolve_type_link("list[str]", self.index, "folio_docs.config")
        assert url is None

    def test_nested_generic(self):
        url = resolve_type_link("list[Optional[Config]]", self.index, "folio_docs.build")
        assert url == "/docs/api-reference/folio_docs/config#config"

    def test_parent_package_lookup(self):
        # From folio_docs.config, look up FunctionIR (in folio_docs.ir)
        # It won't match folio_docs.config.FunctionIR, but folio_docs.FunctionIR won't match either.
        # It should find it via suffix search since FunctionIR is unique.
        url = resolve_type_link("FunctionIR", self.index, "folio_docs.config")
        assert url == "/docs/api-reference/folio_docs/ir#functionir"

    def test_module_name_as_type(self):
        url = resolve_type_link("folio_docs.config", self.index, "folio_docs.build")
        assert url == "/docs/api-reference/folio_docs/config"
