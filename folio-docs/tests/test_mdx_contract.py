...
from folio_docs.extensions import ComponentDefinition, ExtensionRegistry
...
def test_mdx_contract_is_versioned_and_documents_required_props() -> None:
    assert FOLIO_MDX_CONTRACT_VERSION == "1.1"
    ...
    assert components["ApiReferenceIndex"]["props"] == {
        "modules": (
            "Array<{ name: string; description: string; href: string; "
            "classCount: number; functionCount: number }>"
        )
    }
...
def test_rendered_mdx_contract_module_is_importable_typescript() -> None:
    ...
    assert 'export const folioMdxContractVersion = "1.1" as const' in module
...
def _config_core_key_literal() -> set[str]:
    ...
    :mod:`folio_docs.config`; this reads the literal back so the mirror cannot drift
    ...
    """
    tree = ast.parse(
        (ROOT / "folio_docs" / "config.py").read_text(encoding="utf-8")
    )
    ...
    raise AssertionError("No core config-key set literal found in folio_docs/config.py")
