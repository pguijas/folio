...
PROJECT_NAME = "folio-docs"
CLI_ENTRY_POINT_GROUP = "folio.cli"
...
def load_installed_cli_plugins(app: object) -> list[str]:
    ...
    """
    entry_points = sorted(
        importlib.metadata.entry_points(group=CLI_ENTRY_POINT_GROUP),
        key=lambda entry_point: (
            entry_point.name,
            _entry_point_dist_name(entry_point),
        ),
    )
    loaded: list[str] = []
    seen: set[str] = set()
    for entry_point in entry_points:
        if entry_point.name in seen:
            warnings.warn(
                f"Skipping duplicate Folio CLI plugin '{entry_point.name}' from "
                f"'{_entry_point_dist_name(entry_point) or '<unknown distribution>'}'",
                UserWarning,
            )
            continue
        ...
        try:
            ...
            warnings.warn(
                f"Skipping Folio CLI plugin '{entry_point.name}': {exc}",
                UserWarning,
            )
            continue
