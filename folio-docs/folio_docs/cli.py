...
from folio_docs.branding import (
    FOLIO_LOGO_STYLE,
    FOLIO_NEWS_STYLE,
...
from folio_docs.config import DEFAULT_DOCSTRING_STYLE, plugin_config_keys, resolve_output_dir
from folio_docs.features import disabled_feature_message, is_feature_enabled
from folio_docs._github_pages import app as github_pages_app
...
from folio_docs.workflows import github_pages_workflows

app = typer.Typer(
    name="folio",
    help="Documentation and repository-native agent work from one CLI.",
...
_installed_cli_plugins = load_installed_cli_plugins(app)
...
def _generate_docs_yaml(info: dict) -> str:
    ...
    if python_path:
        lines += [
            "  python:",
            "    paths:",
            f"      - {_yaml_scalar(python_path)}",
        ]
        if docstring_style != DEFAULT_DOCSTRING_STYLE:
            lines.append(f"    docstring_style: {_yaml_scalar(docstring_style)}")
        lines += [
            "    # exclude:",
            '    #   - "**/test_*.py"',
        ]
    ...
    lines += [
        ...
        "",
        "theme:",
        f"  preset: {_yaml_scalar(theme_preset)}",
        "  dark_mode: true",
        '  # logo: "docs/logo.png"',
...
def coverage(
    ...
    from folio_docs.config import load_config
    from folio_docs.coverage import aggregate, analyze_modules
