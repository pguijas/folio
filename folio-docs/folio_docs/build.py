...
from folio_docs.branding import folio_banner
from folio_docs.builtins import register_builtin_components
from folio_docs.config import Config, load_config_with_plugins, plugin_config_keys
...
from folio_docs.docs.builtin_drift import check_template_drift
from folio_docs.docs.next_runtime import preflight_check
...
from folio_docs.link_checker import check_links
from folio_docs.parser.markdown_parser import MarkdownResult, parse_markdown_file
from folio_docs.plugin import PluginDocument, PluginManager
...
def _check_bundled_template_drift(template_dir: Path) -> None:
    ...
    :data:`folio_docs.builtins.BUILTIN_COMPONENTS`, so the manifest and
...
def _resolve_overlay_dir(project_dir: Path, config: Config) -> Path:
    ...
    :func:`folio_docs.paths.resolve_contained_dir`: the overlay must live inside the
...
def _build_manifest_context(
    ...
    output_inputs = [
        Path(__file__),
        folio_dir / "config.py",
        folio_dir / "docs" / "site_builder.py",
        folio_dir / "docs" / "template_workspace.py",
        folio_dir / "docs" / "mdx_writer.py",
        folio_dir / "docs" / "sidebar.py",
        folio_dir / "docs" / "xref.py",
        folio_dir / "agents" / "artifacts.py",
        folio_dir / "agents" / "contract.py",
        folio_dir / "agents" / "markdown.py",
        folio_dir / "agents" / "llm_output.py",
        folio_dir / "signatures.py",
...
def _write_meta_pages(
    ...
    # A warm build keeps ``content/`` so unchanged pages can be reused. Its
    # metadata cannot be reused the same way: when a docs directory moves or
    # disappears, no current metadata file overwrites the old path and Nextra
    # rejects entries that point at pages which no longer exist. Clear the
    # generated metadata tree before writing the complete current set. Plugins
    # merge their metadata later in ``emit_assets``.
    builder.remove_meta_tree("")
...
def _start_dev_server(
    *,
    ...
    from folio_docs.watcher import start_watcher
