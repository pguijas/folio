...
def _arg_table(func: FunctionIR) -> str:
    ...
    for arg in func.args:
        if arg.kind == "var_positional":
            ...
            name = arg.name
        # A PEP 604 union carries a pipe, which is the column separator here.
        # Type and default need the same escaping the description already got.
        default = (
            f"`{_table_cell(arg.default)}`" if arg.default is not None else ""
        )
        rows.append(
            f"| `{_table_cell(name)}` | `{_table_cell(arg.type or 'Any')}` | {default} "
...
def _function_blocks(func: FunctionIR, heading: str, project_dir: str) -> list[str]:
    ...
    blocks = [
        ...
        f"```python\n{render_signature(func, show_parens=not is_property)}\n```",
    ]
...
def _doc_blocks(doc: MarkdownResult, config: Config | None) -> list[str]:
    ...
    content = mdx_to_markdown(doc.content).strip()
