...
# 1.1 corrects prop declarations that 1.0 published but the bundled
# components never accepted (Tabs.defaultValue, TabItem.value,
# TimelineItem.description, the ApiReferenceIndex module fields), and adds the
# props they do accept. No component behaviour changed; the contract now
# describes what always shipped. `folio.generator.builtin_drift` compares it
# against the component sources so it cannot drift again.
FOLIO_MDX_CONTRACT_VERSION = "1.1"
...
# The top-level docs.yaml keys Folio itself accepts. `folio_docs.config` owns the
...
def build_contract(
    ...
    :mod:`folio_docs.builtins` the single source of truth for the published
    TypeScript contract.
    """
