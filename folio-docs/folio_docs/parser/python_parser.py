...
def _is_python_excluded(path: Path, excludes: list[str]) -> bool:
    ...
    for raw_exclude in excludes:
        exclude_text = Path(raw_exclude).as_posix().rstrip("/")
        if any(marker in exclude_text for marker in ("*", "?", "[")):
            if fnmatch.fnmatch(candidate_text, exclude_text) or fnmatch.fnmatch(
                candidate_text, f"{exclude_text}/**"
            ):
                return True
            continue

        exclude_path = Path(raw_exclude).resolve()
        if candidate == exclude_path or candidate.is_relative_to(exclude_path):
            return True
    return False
...
def _is_python_import_root(path: Path) -> bool:
    ...
    return path.name == "src" and not (path / "__init__.py").is_file()
...
def parse_python_directory(
    ...
    for py_file in sorted(root.rglob("*.py")):
        if _is_python_excluded(py_file, excludes):
            continue
...
def parse_python_source_root(
    ...
    root = Path(directory)
    modules = [
        parse_python_file(py_file, py_file.stem, style=style)
        for py_file in sorted(root.glob("*.py"))
        if py_file.name != "__init__.py"
        and not _is_python_excluded(py_file, excludes)
    ]
    for package_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        if not any(
            not _is_python_excluded(py_file, excludes)
