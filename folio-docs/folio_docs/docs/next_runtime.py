...
# Minimum toolchain the bundled template needs: Next.js 16 requires
# Node >= 20.19, and the template lockfile is pnpm 10.
MIN_NODE_VERSION = (20, 19)
MIN_PNPM_VERSION = (10, 0)
...
def _parse_version(raw: str) -> tuple[int, ...]:
    parts = raw.strip().lstrip("v").split("-")[0].split("+")[0].split(".")
    numbers: list[int] = []
    for part in parts:
        if not part.isdigit():
            break
...
def preflight_check() -> None:
    ...
    if not shutil.which("node"):
        problems.append(
            "Node.js was not found. The generated site is a Next.js app; "
            "install Node >= 20.19 from https://nodejs.org/"
        )
    else:
        raw = _tool_version_output(["node", "--version"])
        version = _parse_version(raw)
        if version and version < MIN_NODE_VERSION:
            problems.append(
                f"Node.js {raw} is too old - the bundled template "
                "(Next.js 16) requires Node >= 20.19. "
                "Upgrade from https://nodejs.org/"
            )

    if not shutil.which("pnpm"):
        problems.append("pnpm was not found. Install it with: npm install -g pnpm")
    else:
        raw = _tool_version_output(["pnpm", "--version"])
        version = _parse_version(raw)
        if version and version < MIN_PNPM_VERSION:
            problems.append(
                f"pnpm {raw} is too old - the template lockfile requires "
                "pnpm >= 10. Upgrade with: npm install -g pnpm@10"
            )
