...
def landing_sections(raw_sections: Any) -> list[dict[str, Any]]:
    ...
    for raw_section in raw_sections:
        ...
        # Fields every section type may carry, whatever its own normalizer does.
        _normalize_stage(section)
        _normalize_actions(section)
...
def _safe_section_href(raw_value: Any, path: str, default: str) -> str:
    ...
    if not isinstance(raw_value, str) or not raw_value:
        return default
    from folio_docs.config import _theme_href
...
def _normalize_actions(section: dict[str, Any]) -> None:
    ...
    for raw_action in raw_actions:
        if not isinstance(raw_action, dict):
            warnings.warn(
                f"landing: section action {raw_action!r} must be a mapping — dropped",
                UserWarning,
                stacklevel=2,
            )
            continue
        title = _string(raw_action.get("title")).strip()
        if not title:
            warnings.warn(
                f"landing: section action {raw_action!r} needs a title — dropped",
                UserWarning,
                stacklevel=2,
            )
            continue
        href = _safe_section_href(
            raw_action.get("href"), f"landing: section action {title!r} href", ""
        )
        if not href:
            warnings.warn(
                f"landing: section action {title!r} needs an href — dropped",
                UserWarning,
                stacklevel=2,
            )
            continue
        action: dict[str, Any] = {"title": title, "href": href}
        detail = _string(raw_action.get("detail")).strip()
        if detail:
            action["detail"] = detail
        if raw_action.get("primary") is True:
            action["primary"] = True
        if raw_action.get("external") is True:
...
def _normalize_boards_section(section: dict[str, Any]) -> dict[str, Any]:
    ...
    # narrow: true caps the roadmap at a centered max-w-3xl.
...
def normalize_landing(raw_landing: Any) -> dict[str, Any]:
    ...
    normalized_hero: dict[str, Any] = {
        ...
        # Missing keeps the template fallback; an explicit empty string is a
        # deliberate no-kicker hero.
        "tagline": _string(hero.get("tagline")) if "tagline" in hero else None,
