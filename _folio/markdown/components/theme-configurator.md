...
metadata. See [Theme Packages](/docs/theming/theme-packages) for the ownership
model, file overlay rules, and validation checklist.

### Create a Custom Preset

The bundled presets live in the template's `theme/presets.ts` and use the interfaces from `theme/preset-types.ts`. A project adds one without forking the template: a [theme package](/docs/theming/theme-packages#register-a-custom-preset) calls `registerPreset` from its `theme/project-theme.ts`, or a [template overlay](/docs/theming/custom-templates#overlay-partial-override) ships its own copy of `theme/presets.ts`. Folio reads the `id` from either, so `theme.preset` can select it.
