...
| `package` | `string` \| `mapping` | `""` | Theme package copied over the bundled template before generated content and metadata are injected. A string is a directory in the project. A mapping (`git`, `rev`, `digest`, optional `path`) fetches a published package and verifies its tree against the digest. See [Theme Packages](/docs/theming/theme-packages#install-a-package-someone-else-published). |
...
See [Theming](/docs/theming) for the ownership model, safe personalization
...
static export pipeline. See [Custom Templates](/docs/theming/custom-templates) for
...
The [landing page](/docs/plugins/landing), the [roadmap](/docs/plugins/roadmap) and OpenAPI ship in the `folio` binary; [Plugins](/docs/plugins) covers each. Each one activates through its own top-level section in `docs.yaml` and stays inert without it; there is no `plugins:` list.
