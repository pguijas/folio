...
interface PublicLayoutProps {
  ...
  eyebrow?: string
  description?: string
  /** Document-shaped views (roadmap) read in a centered column; canvas-shaped
   * views keep the full width. */
  narrow?: boolean
  /** Tool-shaped views drop the band: a working surface should not spend a
   * screen of chrome on a display title before its first row, and it carries
   * its own name and cross-links in its own toolbar.
   * Reading surfaces keep the band. */
  dense?: boolean
  /** Sibling public views of the same plugin, rendered as quiet
   * cross-links on the band's top row opposite the Home link. */
  links?: { label: string; href: string }[]
  /** App-shaped views (a plugin's workspace mode) take exactly the
...
export function PublicLayout({
  ...
  // view and the page above it line up and none of them is squeezed into a
  ...
  return (
    <main
      ...
      // the same fact `pt-16` encodes — and anything a view offsets against
      // the navbar computes sticky tops and viewport bounds from it.
      // Undeclared, the fallback 0px slides sticky controls under the navbar
    ...
    >
      ...
      {/* Tool surfaces render no band at all: `dense` means the view
          carries its own name and cross-links inside its own toolbar, so
          the page opens on the work rather than on a header. Reading
          surfaces keep the full band below. */}
      ...
      {/* A dense surface holds its own top padding: without a band, a
          toolbar at `pt-0` sits flush against the navbar's bottom edge.

          A workspace goes full-bleed at lg+, not just unpadded vertically:
          a rail drawn flush-left inside a centered, side-padded measure
          floats 24px into whitespace. The workspace holds its own inner air
          instead — the rail touches the viewport, the content column pads
          itself. Below lg the page flows normally. */}
      <section
        className={cn(
          "mx-auto px-6",
          measure,
