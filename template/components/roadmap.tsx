...
/* One feature, one line — a markdown-style task-list item. Shipped
   ...
   (dashed once the phase is far out). */
  ...
  return (
    <li
      className={cn(
        ...
        /* Done first and exclusive. These were additive `&&` clauses, so a
           finished line inside an in-progress phase matched two of them and
           the later class won — which is how an accent-filled box came out
           wearing the pending phase's border. */
        done
          ? "text-foreground/85"
          : status === "active"
            ? "text-foreground/80"
...
interface TimelineProps {
  ...
  compact?: boolean
  maxPhases?: number
  /** The full roadmap's href for the truncation row. Without it the row is a
   * plain count: the component cannot tell which roadmap route, if any, the
   * config publishes. */
  moreLink?: string
}
...
interface RoadmapProps {
   ...
   * description and its links are drawn by the layout's title band. */
  title?: string
