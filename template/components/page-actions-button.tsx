...
function createAssistantReadPrompt(markdownPath?: string) {
  return `Read from ${getMarkdownUrl(markdownPath)} so I can ask questions about it.`
}
...
function actionText(action: PageAction, markdownPath?: string) {
  if (action.kind === "copy") {
    return getPageContext().content
  }
  if (action.kind === "view") {
    return getMarkdownUrl(markdownPath)
  }
  if (action.kind === "mcp") {
    return createMcpPayload()
  }
  return createAssistantReadPrompt(markdownPath)
}
...
function buildAssistantHref(
  ...
  const text = actionText(action, markdownPath)
...
export function PageActionsButton({
  ...
  const handleOpenChange = useCallback((nextOpen: boolean) => {
    if (nextOpen && CHATGPT_ACTION) {
      setAssistantHref(buildAssistantHref(CHATGPT_ACTION, markdownPath))
    }
    setOpen(nextOpen)
  }, [markdownPath])
  ...
  const handleSelect = useCallback((action: PageAction) => {
    setOpen(false)
    if (action.kind === "view") {
      window.open(getMarkdownUrl(markdownPath), "_blank", "noopener,noreferrer")
      return
    }

    copyActionText(action)
  }, [copyActionText, markdownPath])
