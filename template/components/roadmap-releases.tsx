...
import { resolveFeature, sortByVersion, statusLabels } from "@/lib/roadmap-utils"
...
function ListNode({ status }: { status: RoadmapStatus }) {
  return (
    ...
    >
      {status === "shipped" ? (
        <span className="size-2.5 rounded-full bg-primary forced-colors:bg-[color:Highlight]" />
      ) : null}
      {status === "active" ? (
...
function ReleaseRow({
  ...
  const reached = active || phase.status === "shipped"
...
/** One feature, one line, as a checklist: shipped work is checked off,
 *  everything pending is an open box. */
function FeatureItem({
  ...
  /* The phase answers for its lines unless a line answers for itself. A
     release in progress is the case that needs it: some of the work is done
     and drawing all of it as pending understates what is already built. */
  const { text, done } = resolveFeature(feature, status)
  return (
...
function ReleaseDetail({ phase }: { phase: RoadmapPhase }) {
  const active = phase.status === "active"
  /* Shipped and in-progress are both releases the product has reached, so both
     carry the accent. The node shape still tells them apart: a filled disc has
     shipped, a ring with a core is in progress. Grey is reserved for what has
     not been started. */
  const reached = active || phase.status === "shipped"
  return (
    <>
