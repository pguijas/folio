...
/** Turn a root-relative href from config into one relative to the page that
 * renders it, so exports stay portable (file://, GitHub Pages subpaths).
 *
 * `pathToRoot` is the same measure the public views already carry: "." on the
 * site root, ".." one route below it. It defaults to the root, which is where
 * the bundled landing lives; a landing served from a subroute has to pass its
 * own depth or every href it emits resolves one level too deep. */
export function normalizeLandingHref(href: string, pathToRoot = ".") {
  return href.startsWith("/") ? `${pathToRoot}${href}` : href
}
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className={`inline-flex size-8 items-center justify-center rounded-md transition-colors ${copied ? "text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
