...
function FeaturesSection({ section }: { section: LandingSection }) {
  ...
  if (section.variant === "bento") {
    return <FeaturesBentoSection section={section} features={features} />
  }
 ...
 */
function CellsSection({ section }: { section: LandingSection }) {
  ...
  return (
    <section className="landing-section border-b border-border bg-muted/20">
      <div className="mx-auto max-w-site px-6 py-20">
        ...
        >
          {items.map((item) => {
            const href = item.href ? normalizeLandingHref(item.href) : null
...
/**
 * "boards" section: live plugin-rendered miniatures (roadmap + kanban) inside
 * browser chrome. Renders nothing unless at least one of the two board
 * plugins wrote data for this build — the miniatures are real components
 * reading the same generated data modules as the full pages.
 */
function BoardsSection({ section }: { section: LandingSection }) {
  const hasRoadmap = roadmapPhases.length > 0
  // kanban_embed: false keeps the board off the landing; its link moves
  // under the roadmap miniature instead.
  const hasKanban = kanbanColumns.length > 0 && section.kanban_embed !== false
  const kanbanLinkOnly = kanbanColumns.length > 0 && !hasKanban
  if (!hasRoadmap && !hasKanban) {
    return null
  }

  const roadmapUrl = section.roadmap_url || "/roadmap"
  const kanbanUrl = section.kanban_url || "/kanban"
  const roadmapLinkText = section.roadmap_link_text || "Full roadmap"
  const kanbanLinkText = section.kanban_link_text || "Open the board"
  // A single narrow exhibit is centered, so its heading centers with it;
  // the two-board grid keeps the left-anchored editorial heading.
  const centeredExhibit = section.narrow === true && !(hasRoadmap && hasKanban)
      ...
      <div className="mx-auto max-w-site px-6 py-20">
        ...
        <div
          className={cn(
            ...
            hasRoadmap &&
              hasKanban &&
              "lg:grid-cols-[minmax(0,0.44fr)_minmax(0,0.56fr)] lg:items-start",
            // narrow: a single embedded board becomes a centered exhibit
            // instead of a full-bleed frame; no-op with two boards.
              ...
              <BrowserFrame
                ...
                footer={
                  <>
                    ...
                    {kanbanLinkOnly ? (
                      <a
                        href={normalizeLandingHref(kanbanUrl)}
                        className="group inline-flex w-full items-center justify-center gap-1.5 rounded-md bg-foreground px-3 py-2 font-sans text-xs font-semibold text-background transition-colors hover:bg-foreground/80 sm:w-auto sm:py-1.5"
                      >
                        {kanbanLinkText}
                        <span
                          aria-hidden="true"
                    ...
                    <a
                      href={normalizeLandingHref(roadmapUrl)}
...
/**
 * "mechanism" section: config file (with a +/- diff highlight) -> pipeline
 * pill rail -> live board in browser chrome. The code window and commit strip
 * come entirely from config; the board is the kanban plugin's real data.
 */
function MechanismSection({ section }: { section: LandingSection }) {
  const code = typeof section.code === "string" ? section.code : ""
  const hasBoard = kanbanColumns.length > 0
  if (!code && !hasBoard) {
    return null
  }
  ...
  const boardUrl = section.board_url || "/kanban"
  const boardLabel = section.board_label || "● CURRENT"
      ...
      <div className="mx-auto max-w-site px-6 py-20">
        ...
        <div
          className={cn(
            ...
            code &&
              hasBoard &&
              "items-center xl:grid-cols-[minmax(0,1fr)_auto_minmax(0,1.05fr)]"
          )}
        >
          {code ? (
          ...
          {code && hasBoard ? (
            <div
              aria-hidden="true"
              className="flex items-center justify-center gap-2 xl:flex-col xl:px-1"
            >
              {pills.map((pill, index) => (
                <Fragment key={`${pill}-${index}`}>
                  {index > 0 ? (
                    <span className="h-px w-4 bg-border xl:h-4 xl:w-px" />
                  ) : null}
                  <span
                    className={cn(
          ...
          {hasBoard ? (
            <div className="min-w-0 self-stretch">
              <BrowserFrame url={boardUrl} label={boardLabel}>
 ...
 * harnesses a team already uses and the portable project surfaces they share.
 * The visual claims interoperability through repository files, not control,
 * orchestration, or a remote write path.
 */
function HarnessSection({ section }: { section: LandingSection }) {
  ...
  return (
    <section className="landing-section border-b border-border bg-background">
      <div className="mx-auto max-w-site px-6 py-20">
        <SectionHeading
          eyebrow={section.eyebrow ?? "One core, two products"}
          title={
            section.title ??
            "Docs for people. A harness over harnesses for agents."
          }
          description={section.description}
        />

        <div className="mt-10 grid gap-5 lg:grid-cols-[minmax(0,0.34fr)_minmax(0,0.66fr)]">
          <article className="flex min-w-0 flex-col rounded-lg border border-border bg-card p-6">
            <p className="m-0 font-mono text-[10px] tracking-[0.14em] text-primary uppercase">
              Docs generator
            </p>
          ...
          <figure className="relative m-0 min-w-0 rounded-lg border border-primary/35 bg-primary/[0.03] p-6">
            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-primary/20 pb-5">
              ...
              <p className="m-0 rounded-full border border-primary/30 bg-background px-3 py-1.5 font-mono text-[10px] font-semibold text-primary">
                {section.thesis || "One repository contract."}
              </p>
            </div>
 ...
 */
function StatementSection({ section }: { section: LandingSection }) {
  ...
  const actions = (section.actions ?? []).map(normalizeAction)
