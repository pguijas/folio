 ...
 *
 * Every card is open by default, which is what the server renders and
 * therefore what a reader with no JavaScript gets: the whole roadmap, every
 * project. `?product=<key>` narrows that to one — the link a project page
 * uses, so arriving from that project opens on its card instead of on
 * whichever project happens to be declared first.
 *
...
export function RoadmapCards({
   ...
   * open, and the hidden cards would flash. Waiting for `open` to stop being
   * null is waiting for exactly that commit. */
  useEffect(() => {
    if (open === null) return
