...
/** A feature line, resolved: the sentence and whether it is done.
 *
 * A phase's status is the default answer for every line under it, which is
 * right for a shipped phase and for one nobody has started. It is wrong for a
 * release in progress, where some of the work is finished and the rest is not,
 * and where rendering all of it as pending understates what exists. A line may
 * therefore carry its own `done`, and that wins over the phase.
 */
...
export function resolveFeature(
  ...
  if (typeof feature === "string") return { text: feature, done: shipped }
  return { text: feature.text, done: feature.done ?? shipped }
}
