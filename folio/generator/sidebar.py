...
_DOC_PAGE_ORDER: list[tuple] = [
    ...
    # Kanban is a first-party plugin, but its guide is a section of its own:
    # a shipped working surface reads beside the other product sections, not
    # inside the extension mechanism that implements it. It stays adjacent to
    # Plugins, which keeps naming it one.
    (
        "kanban",
        "Kanban",
        [
...
def _apply_nav_order(meta: dict[str, Any], nav: list[str]) -> dict[str, Any]:
    ...
    for key, value in meta.items():
        ...
        aliases[_slugify(key)] = key
        if isinstance(title, str):
            aliases[_slugify(title)] = key
