...
from folio.generator.theme_package_validator import validate_and_raise
from folio.schemas.theme_contract import (
...
logger = logging.getLogger("folio.template")
...
# delete on the next build (see folio.build._materialize_overlay_template).
...
# validation (folio.config._theme_radius) rejects unknown theme.radius values,
...
class TemplateConfigInjector:
    ...
    def _note_skip(self, target: str, reason: str) -> None:
        ...
        :func:`folio.build._resolve_template_dir`, so reaching a skip here always
    ...
    def _plugin_view_owns_root(self) -> bool:
        ...
        Reads `config.extra["kanban"]["routes"]["public"]` to detect when the
        kanban plugin (or future plugins) register a view at `/`. This is checked
        at prepare time (after all configure hooks run), and reads `extra` directly
        because ad-hoc Config attributes do not survive `resolve_paths` — only
        declared fields and `extra` itself are preserved (config.py:136-206).

        Prepare-time reading sees the final state: project plugins may override
        `config.extra["kanban"]` after the kanban plugin's configure hook runs
        (kanban.py:347 documents this), so a configure-time latch could go stale.
        ...
        """
        kanban = self.config.extra.get("kanban")
        if not isinstance(kanban, dict):
            return False
        routes = kanban.get("routes")
        if not isinstance(routes, dict):
            return False
        public_route = routes.get("public")
        if not isinstance(public_route, str):
            return False
        # The same spelling rule the kanban plugin's normalization applies
        # (folio/plugins/kanban.py): an override written after configure may
        # carry a non-normalized path, and registration re-normalizes it, so
        # the injector must agree with what will actually be registered.
...
""".replace("__DOCS_APP_PATH__", docs_app_path)
    ...
    def _inject_sitemap(self) -> None:
        ...
        for site_route in ("sitemap.ts", "robots.ts"):
            ...
            include_docs_index = self.config.landing_enabled or self._plugin_view_owns_root()
            content = content.replace(
