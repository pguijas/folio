"""Cardfile board loader: the ``kanban.source: <dir>/`` v2 board format.
...
def is_board_dir(path: Path) -> bool:
    """True when ``kanban.source`` points at a cardfile board directory."""
    return path.is_dir()
...
def _load_card(path: Path, *, column_ids: list[str]) -> dict[str, Any]:
    ...
    if card_id != _slugify(card_id):
        raise ValueError(
            ...
            f"the card id and must look like '{_slugify(card_id) or 'my-card'}'"
        )
...
def _resolve_artifacts(
    ...
    for card in cards:
        ...
        for raw in raw_artifacts or []:
            ...
            if kind == "url":
                # The same scheme policy every href in docs.yaml gets —
                # enforced at load time so `folio kanban check`, the CLI
                # write path (which revalidates and rolls back), and the
                # build all reject a javascript:/data: target identically.
                from folio.config import _theme_href

                _theme_href(target, f"kanban: card '{card_id}' url artifact")
