...
from folio.config import DEFAULT_DOCSTRING_STYLE, resolve_output_dir
from folio.features import disabled_feature_message, is_feature_enabled
from folio.plugin import normalize_config_key_names
from folio.plugins import roadmap as roadmap_plugin
from folio.workflows import github_pages_workflows

app = typer.Typer(
    name="folio",
    help="A modern Python documentation generator. Sphinx alternative using Nextra + shadcn.",
    no_args_is_help=True,
)
console = Console()
roadmap_plugin.register_cli(app)
...
_INIT_THEME_PRESETS = (
    (
        "organic-editorial",
        "Organic Editorial",
        "Warm editorial docs with rich surfaces.",
    ),
...
def _detected_summary_body(info: dict, target: Path, repo: str) -> Text:
    rows = (
        ...
        ("🧬", "Source", info["python_path"]),
        ("🌐", "Repo", repo or "not detected"),
        ("🐍", "Python", info["python_version"]),
        ("🛠️", "Framework", info["framework"]),
        ("✨", "Status", info["status"]),
    )
    label_width = max(len(label) for _icon, label, _value in rows)
    body = Text()
    ...
    *,
    ...
    if _init_can_use_arrow_select():
        ...
        if arrow_answer is not None:
            return arrow_answer
        console.print(
            "[yellow]readchar is not installed; using numbered input.[/yellow]"
        )
...
def _ask_init_yes_no(title: str, *, default: bool, style: str) -> bool:
    if _init_can_use_arrow_select():
        ...
        if arrow_answer is not None:
            return arrow_answer
        console.print("[yellow]readchar is not installed; using typed input.[/yellow]")
...
def _detect_project(target: Path) -> dict:
    ...
    if pyproject.exists():
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning: Could not read project metadata from pyproject.toml: {e}[/yellow]"
            )
...
def _detect_git_remote(target: Path) -> str:
    ...
    try:
        ...
        console.print(f"[yellow]Warning: Could not detect git remote: {e}[/yellow]")
        return ""
...
def _read_output_dir_best_effort(config_path: Path, target: Path) -> Path:
    ...
    try:
        ...
        console.print(
            f"[yellow]Warning: Could not read output from docs.yaml: {e}[/yellow]"
        )
        return output_dir
    ...
    if isinstance(raw, dict) and isinstance(raw.get("output"), str):
        try:
            ...
            console.print(
                f"[yellow]Warning: Ignoring unsafe output in docs.yaml: {e}[/yellow]"
            )
    return output_dir
...
def _plugin_config_keys(pm: object) -> set[str]:
    keys: set[str] = set()
...
def _version_sync_config(config_path: Path, pm: object) -> dict:
    ...
    for key in _plugin_config_keys(pm):
        if key in raw:
            synced[key] = raw[key]
    return synced
    ...
    *,
    ...
    for i, version in enumerate(cfg.versions):
        ...
        try:
            ...
            failures.append(failure)
            console.print(
                f"  [red]Build failed for {label} -> {output_path}: {e}[/red]"
            )
            typer.echo(f"  Version output: {output_path}")
            continue
        finally:
