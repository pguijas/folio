...
"""
    ...
    assert section["kanban_url"] == "/kanban"
    assert section["roadmap_link_text"] == "Full roadmap"
    assert section["kanban_link_text"] == "Open the board"
    assert section["kanban_embed"] is True
    assert section["narrow"] is False


def test_load_config_landing_boards_section_can_disable_kanban_embed(
    tmp_path: Path,
):
    cfg_file = tmp_path / "docs.yaml"
    cfg_file.write_text(
...
"""
    ...
    (section,) = config.landing_sections
    assert section["kanban_embed"] is False
...
"""
    ...
    (section,) = config.landing_sections
    assert section["roadmap_url"] == "/roadmap"
    assert section["kanban_url"] == "/board"
...
"""
    ...
    assert section["code"] == "kanban:\n+ moved: yes"
    assert section["commits"] == [{"hash": "a3f92c1", "message": "kanban: move a card"}]
    assert section["pills"] == ["push", "build"]
    assert section["board_url"] == "/kanban"
    assert section["board_label"] == "● LIVE"
...
"""
    ...
    assert config.plugins == ["folio.plugins.roadmap"]
...
"""
    ...
    assert sorted(pm.call_isolated("config_keys")) == [
        ["kanban"],
        ["landing"],
        ["roadmap"],
    ]
...
def test_default_plugin_is_inert_without_its_config_key(tmp_path: Path):
    from folio.config import load_config_with_plugins, plugin_config_keys
...
def test_normalize_docs_route_base_rejects_traversal(value: str):
    from folio.config import normalize_docs_route_base
...
def test_theme_header_warns_on_unknown_key():
    from folio.config import _theme_header
...
def test_theme_preview_warns_on_unknown_key():
    from folio.config import _theme_preview
...
def test_theme_variants_combination_limit_boundary_ok():
    from folio.config import _theme_variants
