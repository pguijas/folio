"""The folio kanban command group: write path over a cardfile board."""
...
def _invoke(project: Path, *args: str):
    return runner.invoke(app, ["kanban", *args, "--project-dir", str(project)])
...
def test_check_passes_and_fails(tmp_path: Path) -> None:
    ...
    # no roadmap section in docs.yaml → the milestone registry stays silent
...
def test_write_commands_refuse_without_a_board_directory(tmp_path: Path) -> None:
    ...
    result = _invoke(project, "move", "alpha", "doing")
    assert result.exit_code == 1
    assert "`kanban.source:` pointing at a board directory" in result.output
...
def test_check_refuses_a_columns_key_like_the_build_does(tmp_path: Path) -> None:
    ...
    BOTH a `columns:` key in docs.yaml used to pass check but fail the build."""
...
def test_show_reports_a_legacy_config_without_a_traceback(tmp_path: Path) -> None:
    """A docs.yaml with inline columns used to escape as a traceback; now
...
def test_attach_move_of_a_tracked_source_commits_its_deletion(tmp_path: Path) -> None:
    ...
    result = _invoke(
        project, "attach", "alpha", str(source), "--move", "--label", "Teardown",
        "--commit",
    )
    assert result.exit_code == 0, result.output
...
def test_init_scaffolds_a_board_and_wires_the_config(tmp_path: Path) -> None:
    """From a docs.yaml with no board to a board the other commands accept."""
    ...
    assert "folio kanban move <id> in-progress" in skill
    ...
    config = (project / "docs.yaml").read_text()
...
def test_init_gives_a_board_only_repo_a_front_page(tmp_path: Path) -> None:
    """A repo whose only content is a board has to build, and open on it.

    With no Python and no docs there are no pages at all, and a site with no
    pages does not build: the docs route needs something under it. With no
    `landing:` key the docs index is also the front page, so without this the
    ...
    home = project / "docs" / "index.md"
    assert home.is_file()
    about = home.read_text()
    assert "About this board" in about
    assert "cardfile board" in about
    assert "board/cards/" in about
    assert "board/SKILL.md" in about
    assert "folio kanban --help" in about
    assert "<KanbanBoard />" not in about

    config = (project / "docs.yaml").read_text()
    assert "source:\n  docs:\n    - docs\n" in config
    assert 'public: "/"' in config
    # The board is the front page, so say so rather than send them to /kanban.
    assert "the board is at /\n" in result.output

    # E2E check: the written config crosses the real loader (plugin configure
    # hooks, Task 1 normalization + guards) and lands in extra as "/".
    from folio.config import load_config_with_plugins
    loaded_config, _ = load_config_with_plugins(
        project / "docs.yaml", plugin_base_dir=project
    )
    assert loaded_config.extra["kanban"]["routes"]["public"] == "/"
    # Board loadability (separate concern: cards validate).
    assert _invoke(project, "check").exit_code == 0


def test_init_leaves_an_existing_site_its_own_front_page(tmp_path: Path) -> None:
    """A project that already publishes keeps the board beside its pages."""
    project = tmp_path / "with-docs"
    project.mkdir()
    (project / "docs.yaml").write_text(
        'project:\n  name: "Docs"\nsource:\n  docs:\n    - "docs/"\n',
        encoding="utf-8",
    ...
    result = _invoke(project, "init", "--no-branch")
    assert result.exit_code == 0, result.output
    assert not (project / "docs" / "index.md").exists()
    assert "the board is at /kanban" in result.output


def test_init_replaces_the_untouched_folio_init_stub(tmp_path: Path) -> None:
    """`folio init -y` then `folio kanban init` must open on the board.

    The quickstart runs both inits back to back, so kanban init always finds
    the docs source configured, a Python path that exists only in config, and
    the one-page "Welcome to the documentation" stub. None of that is
    somebody's site — it is what folio init left behind ten seconds earlier,
    and a board-only project wants the board at /, not a stub."""
    ...
    result = _invoke(project, "init", "--no-branch")
    assert result.exit_code == 0, result.output
    about = (project / "docs" / "index.md").read_text()
    assert "About this board" in about
    assert "cardfile board" in about
    assert "<KanbanBoard />" not in about
    assert "the board is at /\n" in result.output
    # The docs source was already wired; a second source block would shadow it.
    config = (project / "docs.yaml").read_text()
    assert config.count("\nsource:\n") == 1
    assert 'public: "/"' in config

    # E2E check: the written config crosses the real loader.
    from folio.config import load_config_with_plugins
    loaded_config, _ = load_config_with_plugins(
        project / "docs.yaml", plugin_base_dir=project
    )
...
def test_init_does_not_replace_a_stub_in_a_subdirectory(tmp_path: Path) -> None:
    """The init stub lives at the docs root; the same bytes anywhere else are
    somebody's page, and replacing them would leave the site with no index."""
    ...
    stub = "# nested\n\nWelcome to the documentation.\n"
    (project / "docs" / "sub" / "index.md").write_text(stub, encoding="utf-8")

    result = _invoke(project, "init", "--no-branch")
    assert result.exit_code == 0, result.output
    assert (project / "docs" / "sub" / "index.md").read_text() == stub
    assert not (project / "docs" / "index.md").exists()
    assert "the board is at /kanban" in result.output


def test_init_ignores_editor_dotfiles_beside_the_stub(tmp_path: Path) -> None:
    """A .DS_Store next to the untouched stub is not somebody's site; the
    cards loader ignores dotfiles and the front-page check follows suit."""
    project = tmp_path / "mac"
    project.mkdir()
    (project / "docs.yaml").write_text(
        'project:\n  name: "mac"\nsource:\n  docs:\n    - "docs/"\n',
        encoding="utf-8",
    ...
    (project / "docs" / ".DS_Store").write_bytes(b"\x00\x01")
...
def test_init_leaves_an_edited_stub_alone(tmp_path: Path) -> None:
    """One changed word makes the stub somebody's page."""
    project = tmp_path / "edited"
    project.mkdir()
    (project / "docs.yaml").write_text(
        'project:\n  name: "edited"\nsource:\n  docs:\n    - "docs/"\n',
        encoding="utf-8",
    ...
    (project / "docs").mkdir()
    stub = "# edited\n\nWelcome to the documentation. Mine now.\n"
    (project / "docs" / "index.md").write_text(stub, encoding="utf-8")

    result = _invoke(project, "init", "--no-branch")
    assert result.exit_code == 0, result.output
    assert (project / "docs" / "index.md").read_text() == stub
    assert "the board is at /kanban" in result.output


def test_init_leaves_real_python_sources_alone(tmp_path: Path) -> None:
    """A Python path that exists on disk means the project publishes an API."""
    project = tmp_path / "pythonic"
    project.mkdir()
    (project / "docs.yaml").write_text(
        'project:\n  name: "pythonic"\n'
        "source:\n  python:\n    paths:\n"
    ...
    result = _invoke(project, "init", "--no-branch")
    assert result.exit_code == 0, result.output
    assert "Welcome to the documentation." in (
...
def test_init_isolates_the_board_on_its_own_branch(tmp_path: Path) -> None:
    ...
    project.mkdir()
    (project / "docs.yaml").write_text('project:\n  name: "Repo"\n', encoding="utf-8")
    for args in (
        ["init", "-q"],
