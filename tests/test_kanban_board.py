"""Cardfile board format (kanban.source pointing at a directory)."""
...
def test_card_pages_are_delisted_from_the_built_sidebar(tmp_path: Path) -> None:
    ...
    from folio.build import _collect_plugin_docs
    ...
    for docs_route in (False, True):
        ...
        manager.register(kanban_plugin, name="folio.plugins.kanban")
...
def test_size_and_source_are_carried_from_cardfile_frontmatter(tmp_path: Path) -> None:
    ...
    assert card["source"] == "folio#feat/artifact-board-poc"
...
def test_board_yaml_icons_map_resolves_onto_cards(tmp_path: Path) -> None:
    ...
    from folio.plugins.kanban import normalize_kanban
