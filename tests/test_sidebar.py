...
def test_plugin_trust_page_exists_for_its_sidebar_entry():
    ...
    from folio.generator.sidebar import _DOC_PAGE_ORDER
...
def test_nested_section_orders_by_declaration_not_alphabetically():
    """A declared child group orders its own pages: the kanban reading order."""
    from folio.parser.markdown_parser import MarkdownResult
    ...
    assert files["kanban/_meta.ts"].splitlines() == [
        "export default {",
        '  "index": {',
        '    "display": "hidden",',
        "  },",
...
def test_order_for_dir_returns_slug_title_pairs_at_every_depth():
    ...
    from folio.generator.sidebar import _order_for_dir

    for path in [(), ("plugins",), ("kanban",)]:
        assert all(len(entry) == 2 for entry in _order_for_dir(path)), path

    assert ("kanban", "Kanban") in _order_for_dir(())
    assert [slug for slug, _ in _order_for_dir(("kanban",))] == [
        "start",
        "formats",
        "cli",
        "agents",
...
def test_order_for_dir_returns_nothing_for_an_undeclared_path():
    ...
    assert _order_for_dir(("kanban", "nope")) == []
