...
def test_folio_experimental_enables_named_feature():
    ...
    assert experimental_enabled(env) is True
...
def test_plugin_api_modules_are_published():
    assert disabled_api_feature_for_module("folio.extensions") is None
    assert disabled_api_feature_for_module("folio.generator.extension_emitter") is None
    assert disabled_api_feature_for_module("folio.plugin") is None
    assert disabled_api_feature_for_module("folio.plugins.roadmap") is None
...
def test_gated_api_modules_stay_gated_regardless_of_mvp_feature_list(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        features, "MVP_DISABLED_API_MODULES", {"folio.plugins.roadmap": "plugins"}
    )
    monkeypatch.delenv("FOLIO_EXPERIMENTAL", raising=False)

    assert disabled_api_feature_for_module("folio.plugins.roadmap") == "plugins"
    # Submodules inherit the gate; unrelated modules do not.
    assert disabled_api_feature_for_module("folio.plugins.roadmap.data") == "plugins"
    assert disabled_api_feature_for_module("folio.plugins.landing") is None
...
def test_folio_experimental_publishes_the_api_modules_it_enables(monkeypatch):
    ...
    assert (
        disabled_api_feature_for_module(
            "folio.plugins.roadmap", {"FOLIO_EXPERIMENTAL": "plugins"}
        )
        is None
    )
    ...
    assert (
        disabled_api_feature_for_module(
            "folio.plugins.roadmap", {"FOLIO_EXPERIMENTAL": "versions"}
        )
        == "plugins"
    )
