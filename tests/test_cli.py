...
def test_cli_init_creates_github_pages_workflow(tmp_path: Path) -> None:
    ...
    assert steps[-2]["run"] == "uv tool run --from folio-docs folio build --clean"
    assert steps[-2]["env"] == {
    ...
    assert workflow["jobs"]["deploy"]["steps"][-1]["uses"] == "actions/deploy-pages@v4"
...
def test_cli_roadmap_is_disabled_in_mvp(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 1
    assert "The 'roadmap' feature is not available in this release" in result.output
