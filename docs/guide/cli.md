...
It prints the installed version, for example `folio 0.3.0`.
...
Preview the source-defined kanban board configured under the `kanban:` key in `docs.yaml` as a table, one section per column. Provided by the first-party default kanban plugin (see the [Kanban](./kanban) page).

```
folio kanban [OPTIONS] [DIRECTORY]
```
...
| Code | Meaning |
|------|---------|
| `0` | The board was printed, or no columns are configured. |
| `1` | Config file was missing, or conflicting directory arguments were passed. |
