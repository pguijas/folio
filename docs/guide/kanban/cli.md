---
title: Kanban CLI
description: Every folio kanban command, what it writes, and the commit it makes.
---

# CLI reference

`folio kanban` is a command group. Bare invocation is a read-only table; the write subcommands (`add`, `move`, `update`, `trail`, `comment`, `attach`) operate boards: one logical operation, one targeted file edit, optionally one conventional commit.
...
| `--config`, `-c` | Config file path. Defaults to `docs.yaml`. |
...
Write commands and `check` require a board directory. When the config has no `kanban.source`, when the source names a file, when it names a missing path, or when a `columns:` key is present, the commands exit 1 and nothing is written — the [migration recipe](/docs/kanban/formats#migrating-to-cards) turns hand YAML into a cardfile board. Edits are targeted line surgery, never a YAML round-trip, so hand formatting and comments survive. Structurally unusual files (block scalars, multiline values, flow-style `artifacts:`) get a loud refusal ("edit the file manually") with the file left untouched.
...
`board.yaml` declares three columns, `cards/` holds one starter card and the copy-me template, `SKILL.md` is the in-repo operating protocol, and a `kanban:` section is appended to `docs.yaml` publishing the board — at `/kanban` beside an existing site, or as the front page (`public: "/"`) when the board is all the project publishes.
...
Refuses rather than overwrites; the four refusal conditions are listed in [Start a board](/docs/kanban/start/). The config is appended to as text, never round-tripped, so comments and hand formatting survive. The generated board is loaded and validated before the command reports success.

This is the only command that writes `docs.yaml`, and it runs before a board exists; see the rules in [Operating a board](/docs/kanban/agents).

### show

```bash
folio kanban [--project-dir PATH]
folio kanban show [DIRECTORY]
```
...
```bash
folio kanban add "Fix flaky test"
folio kanban add "Ship kanban plugin"
folio kanban add "Write release notes"
folio kanban move fix-flaky-test in-progress
```
...
│                   │ ship-kanban-plugin  │ Ship kanban plugin  │      │      │          │         │
...
Validates the cardfile board — the pre-commit and CI gate. Runs the exact normalization the build runs, prints every warning, and exits 1 on the first topology error (the fail list in [Board formats](/docs/kanban/formats#validation)). On success:
...
```bash
folio kanban add TITLE [--status ID] [--description TEXT] [--tags LIST] [--assignee LIST] [--priority LEVEL] [--parent ID] [--commit]
```
...
```bash
folio kanban add "Fix flaky test" --status backlog --tags ci,tests --priority high --commit
```

### move

```bash
folio kanban move CARD_ID STATUS [--after CARD] [--commit]
```
...
All `--after` validation and math run before any write, so a bad anchor leaves the file untouched. See [Ordering](/docs/kanban/formats#ordering-inside-a-column) for the sort rules.

```bash
folio kanban move fix-flaky-test in-progress --commit
folio kanban move fix-flaky-test in-progress --after other-card
```

### update

```bash
folio kanban update CARD_ID --set field=value [--set field=value ...] [--commit]
```

| Flag | Meaning |
| --- | --- |
| `--set field=value` | Repeatable. Only the ten fields below are accepted. |
...
```bash
folio kanban update fix-flaky-test --set assignee=claude --set type=bug
folio kanban update fix-flaky-test --set size=M --set source=folio#fix-flaky
```

### trail

```bash
folio kanban trail CARD_ID --note TEXT [--ref REF] [--actor NAME] [--commit]
```
...
```bash
folio kanban trail fix-flaky-test --note "root cause found" --ref abc1234
```

### comment

```bash
folio kanban comment CARD_ID TEXT [--by NAME] [--commit]
```
...
```bash
folio kanban comment fix-flaky-test "the retry masks the race — see the trail ref" --by peter
```

### attach

```bash
folio kanban attach CARD_ID [PATH] [--move] [--label TEXT] [--commit]
folio kanban attach CARD_ID (--doc PATH | --api SYMBOL | --file PATH | --pr N | --url URL) [--label TEXT] [--commit]
```
...
```bash
folio kanban attach fix-flaky-test .artifacts/flaky-analysis.md --label "Flaky analysis"
folio kanban attach fix-flaky-test /tmp/repro.sh --move
```
...
```bash
folio kanban attach fix-flaky-test --doc docs/research/analysis.md
folio kanban attach fix-flaky-test --url https://example.com/spec --label Spec
```
