...
- `llms.txt` at the site root indexes every published page and `llms-full.txt`
  ...
  off ([Configuration](./configuration#llm)).
...
is the authority on commands and flags, [Configuration](./configuration) on
`docs.yaml` keys, and the [component catalogue](./components/index) on
