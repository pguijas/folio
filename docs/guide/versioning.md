...
Use the CI/CD workflow from the [CI/CD guide](./ci-cd). The important pieces are restoring `_site/` before `folio build-versions`, running without `--clean` for normal deploys, and uploading the complete `_site/` artifact.
