...
The consequence: running **any** `folio` command from a directory whose `docs.yaml` lists plugins executes those plugins' module-level code. `folio --help` is enough. So is a typo that never reaches a command.
...
- **Installing a plugin never activates it.** A `folio` entry point declared by an installed distribution is inert until its name appears in `plugins:`. Entry-point lookup starts from the name you wrote, never from the set of installed distributions (`_find_entry_point` in `folio/plugin.py`).
...
- **Read `docs.yaml` before running `folio` in an unfamiliar repository.** The `plugins:` list is the entire surface.
...
- **Report what looks wrong.** A plugin nobody listed getting loaded, or a file-path plugin resolving outside the project directory, is a bug in Folio. See [SECURITY.md](https://github.com/pguijas/folio/blob/main/SECURITY.md).
