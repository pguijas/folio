"""Project plugin: publish agent-guide.md at the site root.

A human points their coding agent at ``/agent-guide.md`` so the agent can
explain, install, and troubleshoot Folio from the repository's own facts
instead of improvising. Files under the workspace ``public/`` directory pass
through the Next static export untouched, so the guide is fetchable at the
site URL — https://pguijas.github.io/folio/agent-guide.md.

This is not the board protocol: operating a cardfile kanban board is covered
by ``board/SKILL.md``, which the guide points at rather than repeating.
"""
...
DEFAULT_SITE_URL = "https://pguijas.github.io/folio"
DOCS_TOKEN = "{docs}"
...
human to the canonical documentation at {docs}/ for anything it does not
...
Folio has two products over the same repository truth. **Folio Docs** is the
docs generator: it reads Python source and Markdown without executing project
code, then generates the site people read. **Folio for Agents** is the
meta-harness, a harness over harnesses: coding tools share repository rules,
context, board state, and durable artifacts without Folio replacing or
orchestrating them.
...
1. **Repository truth** is the source, guides, rules, board, and artifacts
   already reviewed with the code.
...
5. **Plugins** add components, data, pages,
   and CLI commands. Roadmap, kanban, and landing ship inside Folio and
   switch on when their config key appears in `docs.yaml`; every other
   plugin is listed under `plugins:`.
...
full command and flag reference is at {docs}/cli.
...
- {docs}/configuration is the authority on what `docs.yaml` accepts.
  {docs}/cli is the authority on commands, flags, and exit codes.
...
def render(site_url: str = DEFAULT_SITE_URL) -> str:
    """Render the guide with documentation links rooted at ``site_url``."""
    docs_url = f"{site_url.rstrip('/')}/docs"
    return GUIDE_TEMPLATE.replace(DOCS_TOKEN, docs_url)
...
def emit_assets(builder: Any, config: Any) -> None:
    ...
    site_url = getattr(config, "site_url", "") or DEFAULT_SITE_URL
    (target_dir / GUIDE_FILENAME).write_text(render(site_url), encoding="utf-8")
