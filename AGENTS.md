...
One family, two independent products. Folio Docs builds HTML plus agent-readable
documentation. Folio for Agents is a framework for exchanging and managing
artifacts and work in the repository. They have separate packages, commands, versions, tests, and
release cycles. Positioning, voice, and direction live in `PRODUCT.md` and each
product's own `PRODUCT.md`.

## Source of Truth

- `AGENTS.md` is the canonical project memory for coding agents.
- `CLAUDE.md` must only redirect here.
- Do not create, stage, commit, or ask teammates to upload implementation plans, specs, progress logs, or planning notes into code or release branches. Plan and track work as cards on Folio's dedicated `board` branch instead.
- Folio's operational `board/` directory lives and is updated only on the remote `board` branch. Use a dedicated worktree or checkout for board changes; never recreate or update it on a code or release branch.
- Do not rely on teammate plans as authority. Treat the code, tests, and user-facing documentation as the source of truth.
- Durable project knowledge belongs in well-written code, focused tests, and the relevant product's docs.
...
- **Use exactly two compact stages.** Stage 1 creates standalone candidates in the owning card on the `board` branch and presents them for review without changing the product. Stage 2 starts only after explicit owner confirmation and integrates the selected direction into Folio.
...
- `folio-docs/` - Folio Docs 0.3.0, independently buildable and publishable
  - `folio_docs/` - parser, IR, CLI, build, site generation, and agent-readable docs output
  - `template/` - bundled Nextra + shadcn application
  - `docs/` and `tests/` - Docs-owned guides and test suite
- `folio-agents/` - Folio for Agents 0.1.0, independently buildable and publishable
  - `folio_agents/` - board domain, CLI, and optional Docs publishing adapter
  - `docs/` and `tests/` - Agents-owned guides and test suite
- `docs.yaml` - the unified Folio site build, sourcing both products and
  rendering their independent roadmap tracks
- The root `pyproject.toml` is an integration workspace, not a public product.
- Repository policy, `.github` automation, legal files, contributor guidance,
  and agent instructions stay shared at the monorepo root. Do not duplicate
  repository scaffolding inside product directories in anticipation of a
  future split.
- Folio Docs owns the one bundled frontend template. Folio for Agents may ship
  integration components and assets, but never a second application template.

## Development

```bash
uv sync
uv run pytest folio-docs/tests folio-agents/tests -v
```
...
```bash
cd folio-docs/template && pnpm install && pnpm run dev
```
...
- **Product code boundaries** - `folio-docs`/`folio_docs` 0.3.0 and `folio-agents`/`folio_agents` 0.1.0 install and release separately. Docs owns HTML, Markdown mirrors, LLM indexes, and its authoring contract. Agents owns the board and repository protocol. Neither product imports the other in its core runtime; `folio_agents.integrations.kanban` is an explicit optional adapter. Both products use the single `folio` command: Docs owns the host and installed products contribute commands through the `folio.cli` entry-point group. Folio for Agents registers `folio board`; no product-specific CLI aliases are retained. Tests: `folio-docs/tests/test_product_boundaries.py`, `folio-docs/tests/test_plugin.py`
- **Python source integrity** - Invalid Python fails the build with the source filename instead of silently publishing an empty API module; `source.python.exclude` accepts the documented glob patterns and exact file/directory paths without prefix overmatching; a conventional `src/` source root publishes its top-level modules and packages under their import names rather than a synthetic `src.*` namespace
...
- **Roadmap plugin** - Released first-party default plugin (`folio_docs.docs.integrations.roadmap`). Activated by the `roadmap:` key in the root `docs.yaml`: registers the `Roadmap` component, writes typed `lib/roadmap-data.ts`, publishes the `/docs/roadmap/` page (`routes.docs`, default on) and optional standalone `/roadmap/` view (`routes.public`), and provides the `folio roadmap` CLI table preview. Folio's unified site uses `track` to render Docs and Agents as independent release sequences; a shared build does not imply shared product versions
- **Landing plugin** - Released first-party default plugin (`folio_docs.docs.integrations.landing`). Sole owner of the `landing:` docs.yaml key: its `configure()` hook normalizes the section (bool shorthand, hero variants, CTAs, install/features/sections, opt-in comparison) into the `Config.landing_*` fields consumed by the template injector. `landing.comparison` takes the project's own `{caption, tools, rows}` table, mirroring the `CompareMatrix` prop contract, and carries it into the rendered section; the legacy `comparison: true` still selects Folio's bundled matrix and warns, because that table names Folio's own competitors and only belongs on Folio's pages. Malformed rows drop with a warning rather than failing the build; without the key the site root serves the docs index. The rendered page stays template-bundled (`folio-docs/template/app/page.tsx`) because injection runs before plugin emission hooks
- **Optional Docs board integration** - `folio_agents.integrations.kanban` ships with Folio for Agents, not in Docs' defaults. With both distributions installed and the plugin explicitly listed, it publishes the board through a Folio Docs site. `kanban.ref` reads an independent board branch through a managed detached worktree under `.worktrees/`, using a local branch when present and fetching `origin` in CI; `kanban.sources` maps named product boards into independent canvases without merging planning into the code checkout. The board runtime and `agents.yaml` do not invoke the Docs builder or Node.js; the shared `folio` host discovers `folio board` from the installed Agents distribution. Docs: `folio-agents/docs/`; tests: `folio-agents/tests/`
- **Progressive Agents canvas** - The optional public `KanbanBoard` view is one continuous workspace ordered filters, canvas, selected card, selected artifact. Later context moves right when space permits and below otherwise without changing DOM or keyboard order. One-pixel pointer/keyboard resizers preserve useful reading floors, Escape unwinds one level, `?card=` and `?artifact=` restore the composition, and published artifacts can enter native full screen. The implementation and responsive styles live in Folio for Agents; Folio Docs supplies only the generic plugin host. Docs: `folio-agents/docs/agents/board-component.md`, `folio-agents/docs/agents/board/index.md`; tests: `folio-agents/tests/test_kanban_plugin.py`
- **Cardfile board (kanban source directory)** - `kanban.source` pointing at a directory loads the cardfile format: `board.yaml` holds only the column set, `cards/<id>.md` holds one card each (filename stem = permanent card id; files starting with `_` are skipped). A card may keep a sibling directory `cards/<id>/` for its own output. Markdown/MDX below it is contributed through plugin API 1.1's `collect_docs` hook and compiled at `<docs route>/kanban/<id>/<stem>/` as a normal Folio page; a leading `_` opts out. The whole directory is also republished as a raw bundle at `/_folio/kanban/<id>/` (dotfiles and symlinks excluded, cleared even when kanban is disabled on a warm build), so HTML/CSS/JS/images keep their relative dependencies. Exact resolved-path ownership prevents `../` cross-card claims; card-directory roots and descendants may not be symlinks. A `doc:`/`file:` artifact inside the owned directory links to the compiled page or raw file. Frontmatter carries machine state — `status` (column membership; a one-line diff moves a card), `track` (the workstream used to project independent canvases), `priority`, optional `order` rank, `parent`, `blocked_by`, `created`, typed `artifacts` (`doc`/`api`/`file`/`pr`/`url` + optional label; the board builds no repo URLs — only a `url:` and a target inside the card's own published directory are links, everything else renders as its path) — and the body carries prose: description before the first `##`, `## Acceptance criteria` checkboxes, `## Trail` one-line session entries (`- YYYY-MM-DD @actor (ref): note`, appended at section end). Board topology errors fail the build (unknown status, dangling parent/blocked_by, missing doc/file artifact targets, non-slug filenames); prose problems warn (trail grammar, unknown priority, WIP overflow). Intra-column order is computed (rank, priority, created, id). Loader: `folio-agents/folio_agents/loader.py`; line-surgery editor (`folio-agents/folio_agents/edit.py`) mutates cards by single-line edits with re-parse verification and rollback — never `yaml.safe_dump`. `folio board` is a command group (`folio-agents/folio_agents/cli_commands.py`): bare = table with ids/blocked markers, plus `show`, `check` (CI gate), and write subcommands `add`/`move` (WIP + open-blocker warnings, `--after` rank midpoints)/`update --set`/`trail`/`attach`, all with post-edit board revalidation + rollback and a `--commit` flag emitting conventional `board:` commits scoped to the board dir. The CLI never writes `docs.yaml`. Agent protocol: `board/SKILL.md` (+ `cards/_TEMPLATE.md`); Folio's own instance uses this format only on its dedicated `board` branch. Docs: `folio-agents/docs/agents/board/` (index, start, formats, cli, agents); tests: `folio-agents/tests/test_kanban_board.py`, `folio-agents/tests/test_kanban_edit.py`, `folio-agents/tests/test_kanban_cli.py`
- **Scoped artifact context maintenance** - Board skills recognize the non-CLI directive `condense artifacts <card-id>`. It accepts exactly one permanent card id, reads only that card and its sibling artifact directory, and replaces verbose working context with the objective, confirmed decisions, open choice, canonical artifact links, and validation state. It never edits files or expands to the whole board or repository. Every artifact handoff must include a rendered URL verified to respond successfully; repository and filesystem paths alone do not count. The scaffolded skill and `folio-agents/docs/agents/board/agents.md` carry the same boundary.
...
- **Authoring contract (`/_folio/contract.json`)** - Every build writes one static JSON file into `build_dir/public/_folio/`, carried through the static export like the Markdown mirrors. Payload: the MDX component contract (builtins plus every config/plugin component registered `contract=True`), `configKeys` (`CORE_CONFIG_KEYS` unioned with plugin `config_keys()` claims), and `routes` (emitted docs URLs), under a `folioVersion`/`mdxContractVersion`/`generatedAt` envelope with one instruction to tolerate unknown fields. Deliberately promises no envelope versioning: `mdxContractVersion` versions the components list only. Builders: `folio-docs/folio_docs/agent_output/contract.py` (`build_authoring_contract`/`render_authoring_contract`), written through `AgentArtifacts`. Docs: `docs/guide/plugins/authoring.md`; tests: `tests/test_mdx_contract.py`
...
- **Agent guide (`/agent-guide.md`)** - This repository ships one through the project plugin `docs/plugins/agent_guide.py` (registered in `docs.yaml`, emitted from `emit_assets` like `install_script.py`). It briefs a coding agent that a human asked for help with Folio: concept model in teaching order, install and first run with the real prerequisites, diagnosis recipes keyed by symptom, and rules against inventing config keys or CLI flags. Distinct from `board/SKILL.md` on the `board` branch, which is the agent *operating* Folio's board. Tests: `tests/test_agent_guide_plugin.py`, including an assertion that `folio mcp` stays absent from the CLI so the guide cannot go stale silently
...
- **Never use the mono face for labels.** JetBrains Mono (`font-mono`) is for
  version numerals and literal command text. Eyebrows, status words, section
  labels, layer strings, list ordinals and back-links all take the sans (Sora)
  at a small size, with weight, colour and letter-spacing doing the work
  (owner directive: *"no debemos abusar de esa fuente es horrible"*). The
  uppercase `tracking-[0.14em]` treatment is separable from the face — keep
  the tracking, drop the mono. Count `font-mono` before shipping a component;
  more than two or three distinct roles means it is being abused.
- **A page names itself once.** No header rule repeating a name the body
  heading already carries. The `ViewHeaderRule` mono micro-label above a page
  that titles itself was rejected outright.
- **One background.** A page header sits on the page, not on its own `bg-card`
  slab behind a border — that split reads as two colours stacked. No
  decorative backdrops, and never an ornamental diagram above a real one.
- **Get presence from hierarchy, not size.** Plugin pages take the landing's
  register (`text-3xl sm:text-4xl font-bold`, up to `text-5xl`/`6xl` for one
  hero), never docs-body 15px and never `clamp()` display type. Make the
  in-progress thing dominant, compress what is settled, fade what is far off.
...
- **Build in small steps, one part at a time** (owner directive: *"divide y
  vencerás"*, *"poco a poco"*). Settle the header, then the skeleton, then the
  rows. Do not redesign a whole page in one pass, and do not fan out a dozen
  variants when one careful change was asked for.
- **Show it running, then stop.** Land one change, rebuild, give the link, wait.
  Long plans and large parallel sweeps are slower than they look and usually
  answer a question that was not asked.
- **Prototype inside the built site**, at a real route, never as standalone
  HTML — signing off a mock rendered outside the site does not sign off its
  type scale, and that mistake has already cost a full redesign.
- **The bar is the page it replaces.** Keep the incumbent reachable at a
  second route while a redesign is open, and retire that route with the study
  once the replacement lands — a frozen reference outlives its usefulness the
  moment nobody is comparing against it.
...
- **Always document new features.** Every feature must be documented in the corresponding `folio-docs/docs/guide/` page and tracked in AGENTS.md's "Features Implemented" section. No feature is done until it is documented.
- **No repo plans.** Do not add implementation plans, specs, progress logs, or local backlog files to code or release branches. Folio's cardfile board on the dedicated `board` branch is the only exception: it is the project's work surface, validated by `folio board check`, and the destination for everything the Roadmap Policy keeps out of `docs.yaml`. If a durable decision matters, document the outcome in `folio-docs/docs/guide/` or this file.
...
- **Sidebar ordering.** New doc pages must be added to `_DOC_PAGE_ORDER` in `folio-docs/folio_docs/docs/sidebar.py`, inside the parent entry's children list when the page sits in a subdirectory. No emojis in sidebar titles.
...
- **Where the rest goes.** Maintenance, release work, and fixes become cards in
  `board/` on the dedicated `board` branch, where they carry acceptance criteria
  and a trail. The protocol is `board/SKILL.md` on that branch. Cutting a line
  does not drop the work; it stops selling it.
...
- **Each product owns its release registry.** Folio Docs phases describe Docs;
  Folio for Agents tracks its releases on its own board. Do not create shared
  phases or combine the two products' release checklists.
...
- **Always validate changes.** After implementing a feature or fix, verify it actually works:
  ...
  2. Run `uv run folio serve --verbose` - the unified build must complete without errors and the server must start without crashes
...
- **Primary**: Always develop and test against Folio's OWN docs (`folio-docs/docs/guide/`). The Folio documentation site IS the product showcase - it must be the best presentation.
- **Template dev server**: `cd folio-docs/template && pnpm install && pnpm run dev` - use this for UI/component work.
- **Full pipeline test**: `uv run folio serve --verbose` from the workspace root builds the unified Folio site from Docs and Agents sources.
