...
COMPONENT = Path(__file__).parents[1] / "folio_agents" / "assets" / "kanban-board.tsx"
...
interface RewriteOperation {
  action: "soleValue" | "withSoleValue" | "withOnlyValue" | "roundtrip"
  query: string
  key?: string
  value?: string | null
  cards?: KanbanCard[]
...
const results = operations.map((operation) => {
  ...
  if (operation.action === "withOnlyValue") {
    return withOnlyValue(
      terms,
      operation.key ?? "",
      operation.value ?? null,
    )
...
def test_multi_assignee_matches_either_name(run_queries):
    ...
    answers = run_queries(
        cards, ["assignee:ana", "assignee:bo", "assignee:cara", "-assignee:ana"]
    )
...
CASES: dict[str, list[str]] = {
    ...
    "type:any": ["Alpha"],
    "project:docs": ["Alpha"],
    "project:agents,shared": ["Beta", "Gamma"],
    "project:none": ["Delta", "Epsilon"],
...
def test_rewrite_helpers(tmp_path: Path) -> None:
    ...
    canvas_cases = [
        ("tag:spec -project:docs", "project", "agents", "tag:spec project:agents"),
        ("tag:spec project:docs", "project", None, "tag:spec"),
    ]
    ...
    operations.extend(
        {
            "action": "withOnlyValue",
            "query": query,
            "key": key,
            "value": value,
        }
        for query, key, value, _ in canvas_cases
    )
    operations.extend(
    ...
    for (query, key, value, want), got in zip(
        ...
        assert got == want, (
            f"withSoleValue({query!r}, {key!r}, {value!r}) = {got!r}, want {want!r}"
        )
    offset += len(sole_set_cases)

    for (query, key, value, want), got in zip(
        canvas_cases,
        outputs[offset : offset + len(canvas_cases)],
        strict=True,
    ):
        assert got == want, (
            f"withOnlyValue({query!r}, {key!r}, {value!r}) = {got!r}, want {want!r}"
        )
    offset += len(canvas_cases)
...
def test_filter_options_search_is_substring_and_case_blind(tmp_path: Path) -> None:
    ...
    values = [
        ["feat/plugins-v2", "feat/plugins-v2"],
        ["main", "main"],
        ["FEAT/x", "FEAT/x"],
    ]
    result = subprocess.run(
...
def test_the_inline_md_grammar_is_executed(tmp_path: Path) -> None:
    ...
    assert tokens[0] == [
        {"kind": "text", "text": "run "},
        {"kind": "code", "text": "folio board check"},
        {"kind": "text", "text": " now"},
    ]
    assert tokens[1] == [
