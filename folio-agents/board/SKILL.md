...
description: Use when reading or changing the independent Folio for Agents board, anything touching folio-agents/board/board.yaml or folio-agents/board/cards/*.md, moving an Agents card, appending a trail line, or attaching an artifact.
---

# SKILL: Operate the Folio for Agents board
...
type: bug
milestone: "0.2"                    # optional Folio for Agents release id
...
This board contains Folio for Agents work only. Folio Docs has a separate
board, configuration, milestones, and release cycle. Do not bridge the two
with a `track` field. `milestone` names a Folio for Agents release.
