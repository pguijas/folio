...
interface KanbanCard {
  ...
  tags: string[]
  assignee: string[]
  /** Optional workstream within the board's product. */
  track?: string
  project?: string
  ...
  /** The optional Docs roadmap step this card's milestone names. `phase` is
   * the anchor and `phaseTitle` the human name. */
  phase?: string
  phaseTitle?: string
...
/* Cards are addressed by their stable cardfile id, never by position. */
...
function identify(columns: KanbanColumn[]): BoardColumn[] {
  return columns.map((column) => ({
    ...column,
    cards: column.cards.map((card) => ({ ...card, uid: card.id })),
  }))
}
...
/* repo put it. The export button closes the loop by exporting          */
/* `folio board move` commands. It never writes card files.      */
/* ------------------------------------------------------------------ */
...
/* The board is owned by the card files on disk, so the browser captures
   intent: one `folio board move` command per card whose column drifted from
...
function boardToMoveCommands(baseline: BoardColumn[], board: BoardColumn[]) {
  ...
  for (const column of board) {
    for (const card of column.cards) {
      ...
      const from = committed.get(card.uid)
      if (from !== undefined && from !== column.id) {
        lines.push(`folio board move ${card.id} ${column.id}`)
      }
    }
  }
  return lines.join("\n") + "\n"
...
/* `folio board move <id> in-review` spells it. That is what lets the   */
...
const FILTER_FIELDS: FilterField[] = [
  ...
  },
  {
    key: "project",
    label: "Project",
    values: (card) => (card.project ? [card.project] : []),
  },
  {
    key: "track",
    label: "Track",
    values: (card) => (card.track ? [card.track] : []),
  },
  {
    key: "milestone",
    label: "Milestone",
...
/* Stronger than the generic select above: it replaces every term for the
   field, including exclusions, so clearing a project really clears it and
   choosing one cannot retain a hidden `-project:docs`. */
function withOnlyValue(
  terms: QueryTerm[],
  key: string,
  value: string | null
): string {
  const kept = terms
    .filter((term) => term.field?.key !== key)
    .map(termSource)
...
function FilterPanel({
  ...
  return (
    <div
      ...
      aria-label="Filter"
      role="group"
      /* A rail beside the board, the full height of the reading. A docs
         embed has no column to give on a narrow screen, so its panel becomes
         a drawer pinned to the viewport's left edge.
         ...
         In the workspace the section already owns the composition, so none
         of that machinery earns its keep: the panel is a plain in-flow
         surface with its own scroll, squared off and separated from the
         canvas by WorkspaceDivider. */
      className={cn(
        "bg-card",
        workspace
...
function CardKey({
  ...
  card: KanbanCard
  /* Set only when the board holds more than one project. A card then says
     which one it is on, and saying it is how you filter to it: the chip
     writes `project:<value>` into the same query the rail and the field
     write, instead of a third control above the board owning the choice. */
  ...
  const project = onProject ? card.project?.trim() : ""
  if (parts.length === 0 && !project) {
    return null
  }
  return (
    <span className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-1 font-mono text-[11px] leading-4 tracking-[0.14em] text-primary uppercase">
      {project ? (
        <button
          type="button"
          /* The card opens; this filters. Two jobs on one object is what the
             milestone link was taken off this line for, so the chip is drawn
             as a control and nothing else here is. */
          onClick={(event) => {
            event.stopPropagation()
            onProject?.(project)
          }}
          title={`Filter the board to ${project}`}
          className="rounded-sm border border-primary/30 bg-primary/8 px-1.5 leading-4 text-primary transition-colors hover:border-primary hover:bg-primary/15 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
        >
          {project}
        </button>
      ) : null}
      {parts.length > 0 ? <span className="min-w-0">{parts.join(" · ")}</span> : null}
    </span>
  )
}
...
type WorkspaceLayout = "wide" | "split" | "stacked" | "mobile"
type WorkspaceSurface = "filters" | "card" | "artifact"

const WORKSPACE_SIZE_DEFAULTS = {
  filterWidth: 280,
  filterHeight: 320,
  cardWidth: 760,
  cardHeight: 760,
...
/* Kept with the optional Agents canvas rather than in the Folio Docs
   template. External publication copies this component through the generic
   plugin contract, so the responsive workspace remains owned and versioned
   by Folio for Agents. */
const KANBAN_WORKSPACE_STYLES = `
/* Kanban workspace: one progressive document order at every width.
   Filters lead to the canvas, a selected card follows it, and its artifact
   follows the card. Grid areas move only the visual placement: all four
   surfaces keep that DOM and keyboard order. The one-pixel tracks are the
   visible dividers; their components carry a wider invisible hit target. */
[data-slot="kanban-workspace"] {
  --kanban-filter-divider: 1px;
  --kanban-card-divider: 1px;
  --kanban-artifact-divider: 1px;
  display: flex;
  min-width: 0;
  min-height: 0;
...
[data-slot="kanban-workspace"][data-filter-open="false"] {
  --kanban-filter-width: 0px !important;
  --kanban-filter-height: 0px !important;
  --kanban-filter-divider: 0px;
}

[data-slot="kanban-workspace"][data-card-open="false"] {
  --kanban-card-width: 0px !important;
  --kanban-card-height: 0px !important;
  --kanban-card-divider: 0px;
}

[data-slot="kanban-workspace"][data-artifact-open="false"] {
  --kanban-artifact-width: 0px !important;
  --kanban-artifact-height: 0px !important;
  --kanban-artifact-divider: 0px;
}

[data-slot="kanban-workspace"] [data-slot="kanban-filter-panel"] {
  grid-area: filters;
}

[data-slot="kanban-workspace"] [data-slot="kanban-canvas-shell"] {
  grid-area: canvas;
}

[data-slot="kanban-workspace"] [data-slot="kanban-card-panel"] {
  grid-area: card;
}

[data-slot="kanban-workspace"] [data-slot="kanban-artifact-reader"] {
  grid-area: artifact;
}

[data-slot="kanban-workspace"] [data-slot="kanban-workspace-divider"][data-surface="filters"] {
  grid-area: filter-divider;
}

[data-slot="kanban-workspace"] [data-slot="kanban-workspace-divider"][data-surface="card"] {
  grid-area: card-divider;
}

[data-slot="kanban-workspace"] [data-slot="kanban-workspace-divider"][data-surface="artifact"] {
  grid-area: artifact-divider;
}

@media (max-width: 767px) {
  [data-slot="kanban-workspace"] [data-slot="kanban-filter-panel"] {
    max-height: var(--kanban-filter-height);
    flex: 0 0 var(--kanban-filter-height);
  }

  [data-slot="kanban-workspace"] [data-slot="kanban-canvas-shell"] {
    min-height: min(42rem, 78dvh);
    flex: 0 0 min(42rem, 78dvh);
  }

  [data-slot="kanban-workspace"] [data-slot="kanban-card-panel"] {
    max-height: var(--kanban-card-height);
    flex: 0 0 var(--kanban-card-height);
  }

  [data-slot="kanban-workspace"] [data-slot="kanban-artifact-reader"] {
    min-height: var(--kanban-artifact-height);
    flex: 0 0 var(--kanban-artifact-height);
  }
}
...
@media (min-width: 768px) and (max-width: 1535px) {
  [data-slot="kanban-workspace"] {
    display: grid;
    grid-template-areas:
      "filters filter-divider canvas"
      "filters filter-divider card-divider"
      "filters filter-divider card"
      "filters filter-divider artifact-divider"
      "filters filter-divider artifact";
    grid-template-columns:
      var(--kanban-filter-width)
      var(--kanban-filter-divider)
      minmax(28rem, 1fr);
    grid-template-rows:
      minmax(30rem, 1fr)
      var(--kanban-card-divider)
      var(--kanban-card-height)
      var(--kanban-artifact-divider)
      var(--kanban-artifact-height);
    overflow: auto;
  }
}
...
@media (min-width: 1536px) and (max-width: 2559px) {
  [data-slot="kanban-workspace"] {
    display: grid;
    grid-template-areas:
      "filters filter-divider canvas card-divider card"
      "filters filter-divider artifact-divider artifact-divider artifact-divider"
      "filters filter-divider artifact artifact artifact";
    grid-template-columns:
      var(--kanban-filter-width)
      var(--kanban-filter-divider)
      minmax(28rem, 1fr)
      var(--kanban-card-divider)
      var(--kanban-card-width);
    grid-template-rows:
      minmax(24rem, 1fr)
      var(--kanban-artifact-divider)
      var(--kanban-artifact-height);
...
@media (min-width: 2560px) {
  [data-slot="kanban-workspace"] {
    display: grid;
    grid-template-areas:
      "filters filter-divider canvas card-divider card artifact-divider artifact";
    grid-template-columns:
      var(--kanban-filter-width)
      var(--kanban-filter-divider)
      minmax(28rem, 1fr)
...
type WorkspaceSizes = typeof WORKSPACE_SIZE_DEFAULTS

/* The progressive workspace has the same one-pixel rule between every
   surface. It is also the resize target: a six-pixel hit area reaches past
   the visible hairline without making the rule itself heavier. The DOM never
   changes when CSS moves the artifact from the right to below, so only the
   separator's orientation follows the responsive layout. */
function WorkspaceDivider({
  ...
  onReset: (surface: WorkspaceSurface) => void
}) {
  return (
    <div
      data-slot="kanban-workspace-divider"
      ...
      tabIndex={0}
      onPointerDown={(event) => onPointerDown(event, surface)}
      onKeyDown={(event) => onKeyDown(event, surface)}
      onDoubleClick={() => onReset(surface)}
      className={cn(
        "group relative bg-border focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring",
    ...
    >
      <span
        aria-hidden="true"
        className={cn(
          "pointer-events-none absolute rounded-full bg-muted-foreground/55 opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100",
          orientation === "vertical"
            ? "top-1/2 left-1/2 h-12 w-1.5 -translate-x-1/2 -translate-y-1/2"
            : "top-1/2 left-1/2 h-1.5 w-12 -translate-x-1/2 -translate-y-1/2"
        )}
      />
    </div>
  )
...
/* The reader stays on the board. In the public workspace it is the next
   in-flow surface after the card; a document embed keeps the legacy drawer.
   A `page` artifact is the docs page the build already compiled — fetched
   and unwrapped to its article body, which arrives styled because both pages
   share one stylesheet bundle; rendering markdown here instead would be a
   second pipeline to keep honest. An `asset` artifact is the raw published
   file in a sandboxed frame: prototypes stay live and their scripts never
   reach the board. The workspace can hand either mode to the browser's
   native full-screen surface without replacing its reading URL. */
function ArtifactReader({
  ...
  // Focus moves into the reader: onto the document scroller when there is
  ...
  const toggleFullscreen = async () => {
    try {
      ...
      // Fullscreen can be denied by the browser or an embedding policy; the
      // in-flow reader remains fully usable at its current size.
    }
  }
  ...
  // this document — the browser writes the hash and scrolls the reader, no
  ...
  return (
    <aside
      ref={readerRef}
      data-slot={
        workspace ? "kanban-artifact-reader" : "kanban-artifact-drawer"
      }
      data-workspace-surface={workspace ? "artifact" : undefined}
      aria-label={`Reading ${label}`}
      className={cn(
        "fullscreen:h-dvh fullscreen:w-dvw flex min-h-0 min-w-0 flex-col overflow-hidden bg-card",
        workspace
          ? "h-full"
          : cn(
              "absolute inset-y-0 left-0 border-r border-border shadow-2xl",
    ...
    >
      <header className="flex shrink-0 items-center gap-3 border-b border-border px-5 py-3">
        ...
        <button
          type="button"
          onClick={workspace ? toggleFullscreen : onToggleWide}
          aria-pressed={workspace ? fullscreen : wide}
          className={cn(
          ...
          if (token.href.startsWith("./")) {
            ...
            if (!links || !resolved) {
              ...
              return (
                <Fragment
                  key={index}
                >{`[${token.text}](${token.href})`}</Fragment>
              )
            }
...
function BoardCard({
  ...
  // criteria, trail, artifacts, metadata — lives in the surface that opens
  // on click. It is a dialog only outside the public workspace.
  return (
    <div
      role="listitem"
      data-slot="kanban-card"
    ...
    >
      ...
      <CardKey card={card} onProject={onProject} />
...
/* The complete card, opened by clicking its title on the board. The public
   workspace renders it after the canvas in normal flow; a document embed
   keeps the modal dialog so the board stays one instance. Only that legacy
   modal traps focus: every public workspace surface remains reachable. */
function CardDetail({
  card,
  columns,
  columnIndex,
  ...
  onWorkspaceResizeKey,
  onWorkspaceResizeReset,
  initialArtifact,
}: {
  card: IdentifiedCard
  columns: BoardColumn[]
  ...
  onWorkspaceResizeReset: (surface: WorkspaceSurface) => void
  /** Artifact target restored from the URL: the board resolved it before
   * opening the card, and the reader opens with it. */
  initialArtifact?: string | null
}) {
  const titleId = `${useId().replace(/:/g, "")}-title`
  // For the legacy modal the trap encloses the wrapper, not the dialog
  // frame: its reader is a sibling, and Tab walks both as one layer.
  const wrapperRef = useRef<HTMLDivElement>(null)
  const closeRef = useRef<HTMLButtonElement>(null)
  const bandRef = useRef<HTMLDivElement>(null)
  const readerOpenerRef = useRef<HTMLElement | null>(null)
  // Which artifact is being read, by band index, and at which legacy drawer
  // width. Restored from the URL once, on mount: the deep link promised a
  // reading position, and the card opened to honor it.
  const [reading, setReading] = useState<{
    ...
    return mode ? { index, wide: !workspace && mode === "asset" } : null
  })
  ...
  useEffect(() => {
    ...
    onReaderOpenChange(reading !== null)
  }, [reading, onReaderOpenChange])

  // The reading position is a URL: ?card= restores the selected card and
  // ?artifact= extends the progressive chain. Built from the current
  // location so filter parameters survive. Closing the workspace reader
  // removes only its artifact and keeps the card selected; closing the card
  // is the parent's job. The hash names the document being opened or closed.
  const writeReaderUrl = useCallback(
    (target: string | null) => {
      try {
        ...
        // History unavailable: the reader still opens.
      }
    },
    [card.uid, workspace]
  )
  ...
  const openReader = useCallback(
    (index: number, opener: HTMLElement | null) => {
      ...
      // The width flag only belongs to the legacy drawer. The workspace
      // delegates sizing to the divider and native full-screen action.
      setReading({ index, wide: !workspace && mode === "asset" })
      writeReaderUrl(artifact.target)
    },
    [card.artifacts, workspace, writeReaderUrl]
  )
  ...
  // Closing the card closes the reading with it; the parent then removes
  // the card from the URL as the second half of the progressive unwind.
  ...
  // reader it named had finished opening.
  ...
  useEffect(() => {
    const opener =
      document.activeElement instanceof HTMLElement
        ...
        : null
    // A deep link opened the reader with the card; focus belongs in the
    // reader then, and it has already taken it.
    if (!readingRef.current) {
      closeRef.current?.focus()
    }
    const handleKey = (event: KeyboardEvent) => {
      /* A control inside the card that consumed this key — the open
      ...
      if (event.key === "Escape") {
        // One press, one level: the reader first, then the card.
        if (readingRef.current) {
          closeReader()
          return
        }
  ...
  }, [closeDialog, closeReader, workspace])
  ...
  // openReader above — a document link in the prose reads in the reader,
  ...
  if (card.track) {
    fields.splice(1, 0, ["Track", card.track])
  }
  if (card.project) {
    fields.splice(1, 0, ["Project", card.project])
  }
  if (card.size) {
    fields.push([
      "Size",
  ...
  return (
    <div
      ref={wrapperRef}
      data-slot={workspace ? "kanban-card-workspace" : "kanban-card-dialog"}
      className={
        workspace
          ? "contents"
          : "fixed inset-0 z-50 flex items-center justify-center bg-scrim/55 p-4 backdrop-blur-[1px]"
      }
      onClick={(event) => {
        if (workspace) {
          return
    ...
    >
      {workspace ? (
        <WorkspaceDivider
          surface="card"
          orientation={
            workspaceLayout === "wide" || workspaceLayout === "split"
              ? "vertical"
              : "horizontal"
          }
          onPointerDown={onWorkspaceResizeStart}
          onKeyDown={onWorkspaceResizeKey}
          onReset={onWorkspaceResizeReset}
        />
      ) : null}
      <div
        role={workspace ? "complementary" : "dialog"}
        aria-modal={workspace ? undefined : true}
        aria-labelledby={titleId}
        tabIndex={-1}
        data-slot={workspace ? "kanban-card-panel" : undefined}
        data-workspace-surface={workspace ? "card" : undefined}
        ...
        className={cn(
          ...
          workspace
            ? "h-full"
            : "max-h-[92vh] w-full max-w-6xl rounded-xl border border-border shadow-2xl"
        )}
      >
        <div
          className={cn(
            "flex shrink-0 items-start justify-between gap-4 border-b border-border",
            workspace ? "px-5 py-3" : "px-8 py-4"
          )}
        >
          ...
          <h2
            id={titleId}
            className={cn(
              "m-0 min-w-0 font-bold tracking-[-0.015em] text-balance text-foreground",
              workspace ? "text-lg leading-6" : "text-[22px] leading-[30px]"
            )}
          >
            {card.title}
          </h2>
          {/* Two marks, one 28px height: the hint and the close. A pen sat
              ...
              Card field in the rail names the file, and `folio board`
        ...
        <div
          ...
          className={cn(
            "min-h-0 overflow-y-auto focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring",
            workspace
              ? "flex-1"
              : "grid md:grid-cols-[1fr_20rem] md:grid-rows-[minmax(0,1fr)] md:overflow-y-hidden"
          )}
        >
          <div
            ...
            className={cn(
              ...
              workspace ? "px-5 py-5" : "px-8 py-7 md:overflow-y-auto"
            )}
          >
            {card.description ? (
              ...
              <div
                className={cn(
                  "max-w-[70ch] break-words text-muted-foreground",
                  workspace
                    ? "mb-5 text-sm leading-6"
                    : "mb-7 text-base leading-7"
                )}
              >
          ...
          <aside
            ...
            className={cn(
              "border-t border-border bg-background focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring",
              workspace
                ? "px-5 py-5"
                : "px-7 py-7 md:overflow-y-auto md:border-t-0 md:border-l"
            )}
          >
        ...
        {comments.length > 0 ? (
          <div
            ...
            aria-label="Comments"
            className={cn(
              "max-h-44 shrink-0 overflow-y-auto border-t border-border py-4 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring",
              workspace ? "px-5" : "px-8"
            )}
          >
              ...
              {canAttach && card.id ? (
                <span className="inline-flex min-w-0 items-center rounded-md border border-dashed border-border px-2.5 py-1.5 text-xs text-muted-foreground">
                  ...
                  <code className="ml-1.5 font-mono text-[11px] break-all">
                    {`folio board attach ${card.id} --doc <path>`}
                  </code>
                </span>
              ) : null}
            </div>
      ...
      {/* In a document embed the reader keeps the legacy drawer treatment.
          In the public workspace it is the next in-flow surface after the
          card; CSS places it farther right when the viewport can hold the
          whole chain, and below otherwise. */}
        ...
        <>
          {workspace ? (
            <WorkspaceDivider
              surface="artifact"
              orientation={
                workspaceLayout === "wide" ? "vertical" : "horizontal"
              }
              onPointerDown={onWorkspaceResizeStart}
              onKeyDown={onWorkspaceResizeKey}
              onReset={onWorkspaceResizeReset}
...
export function KanbanBoard({
  ...
  roadmapHref?: string
  /** The standalone public page (/kanban) is an app, not a document. Its
   * stable surface order is filters, canvas, selected card, artifact. CSS
   * lays later context farther right when space permits and below otherwise;
   * one-pixel resizers divide the surfaces. Only the plugin's emitted public
   * view sets this. A board embedded in docs prose keeps the legacy layout. */
  workspace?: boolean
}) {
  ...
  // The public workspace starts with its first step visible: filters lead
  // into the canvas. A docs embed keeps the opt-in panel it always had.
  // The expression remains the only filter state and lives in the URL.
  const [panelOpen, setPanelOpen] = useState(workspace)
  const [workspaceLayout, setWorkspaceLayout] =
    useState<WorkspaceLayout>("split")
  const [workspaceSizes, setWorkspaceSizes] = useState<WorkspaceSizes>(
    WORKSPACE_SIZE_DEFAULTS
  )
  ...
  // handed to the card once and cleared when it closes; CardDetail owns the
  // reading from there.
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(() => {
      ...
      if (!compact) {
        ...
        // ?card= restores the selected card. ?artifact= may extend that
        // progressive chain by one surface, but a stale artifact does not
        // invalidate a real card. The URL is left exactly as it arrived,
          ...
          if (cardParam) {
            ...
            if (owner) {
              ...
              if (artifactParam) {
                ...
                if (artifact && readerMode(artifact)) {
  ...
  const writeCardUrl = useCallback((uid: string | null) => {
    try {
      const url = new URL(window.location.href)
      url.searchParams.delete("card")
      url.searchParams.delete("artifact")
      url.hash = ""
      if (uid) {
        url.searchParams.set("card", uid)
      }
      window.history.replaceState(null, "", `${url.pathname}${url.search}`)
    } catch {
      // History unavailable: selecting the card still updates the workspace.
    }
  }, [])
  ...
  const openDetail = useCallback(
    ...
    [workspace, writeCardUrl]
  )

  const closeDetail = useCallback(() => {
    setSelectedUid(null)
    // Reopening the same card by hand starts without the reader: the deep
    // link was honored once, when it arrived.
    setRestoredArtifact(null)
    setReaderOpen(false)
    if (workspace) {
      writeCardUrl(null)
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      if (event.key !== "/" || (selectedUid !== null && !workspace)) {
        return
      }
      const active = document.activeElement
  ...
  const activeProject = soleValue(queryTerms, "project")

  /* The board used to carry a row of canvas buttons above the filter field,
     a third control setting the product on top of the rail's own select and
     the field itself. It is gone: a card says which project it is on, and
     saying it is how you filter to it. Pressing the chip of the project
     already filtered to clears the filter, so the chip is the whole round
     trip.

     Undefined on a single-project board, where filtering to the only project
     there is would change nothing and the chip would be noise. */
  const filterToProject = useMemo(() => {
    if (projectOptions.length < 2) {
      return undefined
    }
    return (value: string) => {
      const already = activeProject === value.toLowerCase()
      applyQuery(withOnlyValue(queryTerms, "project", already ? null : value))
    }
  }, [projectOptions.length, activeProject, queryTerms, applyQuery])
  ...
  useEffect(() => {
    if (!panelOpen || (!workspace && selectedUid !== null)) {
      return
    }
    ...
    const onPointer = (event: MouseEvent) => {
      if (workspace || window.matchMedia("(min-width: 64rem)").matches) {
        return
      }
  ...
  }, [panelOpen, selectedUid, workspace])
  ...
  const workspaceOrientationFor = (
    surface: WorkspaceSurface
  ): "horizontal" | "vertical" => {
    if (surface === "filters") {
      return workspaceLayout === "mobile" ? "horizontal" : "vertical"
    }
    if (surface === "card") {
      return workspaceLayout === "wide" || workspaceLayout === "split"
        ? "vertical"
        : "horizontal"
    }
    return workspaceLayout === "wide" ? "vertical" : "horizontal"
  }

  const workspaceSizeKey = (
    surface: WorkspaceSurface
  ): keyof WorkspaceSizes => {
    const horizontal = workspaceOrientationFor(surface) === "horizontal"
    if (surface === "filters") {
      return horizontal ? "filterHeight" : "filterWidth"
    }
    if (surface === "card") {
      return horizontal ? "cardHeight" : "cardWidth"
    }
    return horizontal ? "artifactHeight" : "artifactWidth"
  }

  const adjustWorkspaceSize = (surface: WorkspaceSurface, delta: number) => {
    const key = workspaceSizeKey(surface)
    const limits: Record<keyof WorkspaceSizes, [number, number]> = {
      filterWidth: [224, 440],
      filterHeight: [280, 560],
      cardWidth: [440, 760],
      cardHeight: [420, 760],
      artifactWidth: [560, 1040],
      artifactHeight: [480, 900],
    }
    // The filters divider follows that surface; the card and artifact
    // dividers precede theirs. Moving the handle forward therefore grows
    // filters and shrinks either child surface.
    const signed = surface === "filters" ? delta : -delta
    const [minimum, maximum] = limits[key]
    setWorkspaceSizes((current) => ({
      ...current,
      [key]: Math.min(maximum, Math.max(minimum, current[key] + signed)),
    }))
  }
  ...
  const beginWorkspaceResize = (
    ...
    event.preventDefault()
    const orientation = workspaceOrientationFor(surface)
    let previous = orientation === "vertical" ? event.clientX : event.clientY
    const onMove = (move: PointerEvent) => {
      const next = orientation === "vertical" ? move.clientX : move.clientY
      adjustWorkspaceSize(surface, next - previous)
      previous = next
    }
    const onUp = () => {
      window.removeEventListener("pointermove", onMove)
  ...
  const nudgeWorkspaceResize = (
    ...
    event.preventDefault()
    adjustWorkspaceSize(surface, event.key === forward ? 24 : -24)
  }

  const resetWorkspaceSize = (surface: WorkspaceSurface) => {
    const key = workspaceSizeKey(surface)
    setWorkspaceSizes((current) => ({
      ...current,
      [key]: WORKSPACE_SIZE_DEFAULTS[key],
    }))
  }
      ...
      <CardDetail
        ...
        moveCommand={
          committedColumns(baseline).get(selectedCard.uid) !==
            ...
            ? `folio board move ${selectedCard.id} ${board[selectedColumn].id}`
            : ""
        }
        onMove={(target) => moveCard(selectedCard.uid, target)}
        onClose={closeDetail}
        onReaderOpenChange={setReaderOpen}
        onWorkspaceResizeStart={beginWorkspaceResize}
        onWorkspaceResizeKey={nudgeWorkspaceResize}
        onWorkspaceResizeReset={resetWorkspaceSize}
  ...
  return (
    <section
      ref={sectionRef}
      data-slot="kanban"
      /* Workspace mode: from tablet widths the section is exactly the
         viewport below the fixed navbar. Its internal grid scrolls when the
         progressive chain continues below; on a phone the same surfaces
         remain in normal page flow. */
      className={cn(
        "not-prose",
        workspace &&
          "md:h-[calc(100dvh-var(--nextra-navbar-height,0px))] md:min-h-0"
      )}
      aria-label={title || "Kanban board"}
      aria-describedby={description ? descriptionId : undefined}
    >
      <style>{KANBAN_WORKSPACE_STYLES}</style>
      ...
      <div
        inert={
          selectedUid !== null && !compact && !workspace ? true : undefined
        }
        className={workspace ? "md:h-full md:min-h-0" : undefined}
      >
        ...
        {/* The staging card sits above the whole surface except in workspace
            mode, where it belongs to the canvas surface and must not interrupt
            the filters → canvas → card → artifact order. One JSX value, two
        ...
        {/* A compact board is embedded at a width nobody can predict — a
            ...
            In workspace mode this is the layout owner. The DOM always reads
            filters, canvas, card, artifact; responsive grid areas continue
            that chain to the right or below. One-pixel divider tracks replace
            gaps and borders between the surfaces. */}
        <div
          data-slot={workspace ? "kanban-workspace" : undefined}
          data-filter-open={workspace ? panelOpen : undefined}
          data-card-open={workspace ? selectedCard !== null : undefined}
          data-artifact-open={workspace ? readerOpen : undefined}
          style={
            workspace
              ? ({
                  "--kanban-filter-width": `${workspaceSizes.filterWidth}px`,
                  "--kanban-filter-height": `${workspaceSizes.filterHeight}px`,
                  "--kanban-card-width": `${workspaceSizes.cardWidth}px`,
                  "--kanban-card-height": `${workspaceSizes.cardHeight}px`,
                  "--kanban-artifact-width": `${workspaceSizes.artifactWidth}px`,
                  "--kanban-artifact-height": `${workspaceSizes.artifactHeight}px`,
        ...
        >
          ...
          {workspace && panelOpen ? (
            <WorkspaceDivider
              surface="filters"
              orientation={workspaceOrientationFor("filters")}
              onPointerDown={beginWorkspaceResize}
              onKeyDown={nudgeWorkspaceResize}
              onReset={resetWorkspaceSize}
            />
          ) : null}
          {/* The canvas surface is a flex column with one growing child, so
              the staging card, toolbar and diagnosis keep their height while
              the columns scroll in what remains. Its small internal padding
              gives the board air without turning the surface into a window. */}
          <div
            data-slot={workspace ? "kanban-canvas-shell" : undefined}
            data-workspace-surface={workspace ? "canvas" : undefined}
            className={cn(
              "min-w-0",
              workspace && "flex min-h-0 flex-col overflow-hidden px-4 pt-4"
            )}
          >
            {workspace ? stagingCard : null}
                  ...
                  >
                    ...
                    <input
                      ...
                      onKeyDown={(event) => {
                        ...
                        // the card surface later in the workspace.
                        event.stopPropagation()
                        if (query) {
                          applyQuery("")
                        }
            ...
            {/* The canvas grows inside its surface and scrolls both axes, so
                a narrow allocation pans the columns instead of crushing them. */}
            <div
              className={
                cn(
                  compact && "@container",
                  workspace && "min-h-0 flex-1 overflow-auto"
                ) || undefined
              }
            >
              <div
